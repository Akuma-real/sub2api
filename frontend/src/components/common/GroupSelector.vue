<template>
  <div>
    <label class="input-label">
      ...
      <span class="font-normal text-muted-soft">{{
        t("common.selectedCount", { count: modelValue.length })
      }}</span>
    </label>
    <div
      v-if="isSearchable"
      class="flex items-center gap-2 rounded-t-lg border border-b-0 border-hairline bg-surface-soft px-3 py-2"
    >
      <Icon name="search" size="sm" class="shrink-0 text-muted-soft" />
      <input
        v-model="searchText"
        type="text"
        :placeholder="t('common.searchPlaceholder')"
        class="flex-1 bg-transparent text-sm text-ink placeholder:text-muted-soft focus:outline-none"
      />
    </div>
    <div
      :class="[
        ...
        isSearchable
          ? 'rounded-b-lg border border-t-0 border-hairline bg-surface-soft '
    ...
    >
      <label
        ...
        :key="group.id"
        class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 transition-colors hover:bg-canvas"
        :title="
      ...
      >
        <input
          ...
          "
          class="h-3.5 w-3.5 shrink-0 rounded border-hairline text-primary-500 focus:ring-primary-500"
        />
        <GroupBadge
          :name="group.name"
        ...
        />
        <span class="shrink-0 text-xs text-muted-soft">{{
