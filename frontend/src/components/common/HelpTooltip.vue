...
<template>
  ...
  >
    ...
    <slot name="trigger">
      <svg
        class="h-4 w-4 cursor-help text-muted-soft transition-colors hover:text-primary-600"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
    ...
    <Teleport to="body">
      ...
      >
        ...
        <slot>{{ content }}</slot>
        <div
          class="absolute -bottom-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-surface-dark"
        ></div>
      </div>
    </Teleport>
  </div>
