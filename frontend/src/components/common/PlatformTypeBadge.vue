<template>
  <div class="inline-flex flex-col gap-0.5 text-xs font-medium">
    ...
    <div class="inline-flex items-center overflow-hidden rounded-md">
      <span
        :class="['inline-flex items-center gap-1 px-2 py-1', platformClass]"
      >
    ...
    >
      <span
        v-if="planLabel"
        :class="['inline-flex items-center gap-1 px-1.5 py-1', planBadgeClass]"
      >
        <span>{{ planLabel }}</span>
      </span>
      <span
    ...
    <div
      v-if="expiresLabel"
      class="text-[10px] leading-tight text-muted-soft pl-0.5"
...
const privacyBadge = computed(() => {
  ...
  if (props.platform !== "openai" && props.platform !== "antigravity")
    return null;
