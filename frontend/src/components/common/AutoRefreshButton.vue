<template>
  <div class="relative" ref="dropdownRef">
    <button
      ...
      class="inline-flex items-center gap-1.5 rounded-lg border border-hairline bg-canvas px-2.5 py-1.5 text-xs font-medium text-body shadow-sm transition-colors hover:bg-surface-soft"
    ...
    <div
      v-if="showDropdown"
      class="absolute right-0 z-20 mt-1 w-44 rounded-lg border border-hairline bg-canvas shadow-card"
    >
      <div class="p-1.5">
        <button
          @click="$emit('update:enabled', !enabled)"
          class="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-sm text-body hover:bg-surface-card"
        >
