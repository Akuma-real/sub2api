<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      ...
      >
        <div class="space-y-3">
          ...
          >
            <div class="h-4 w-20 animate-pulse rounded bg-hairline"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-hairline"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-hairline pt-3">
            <div class="h-8 w-full animate-pulse rounded bg-hairline"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-hairline bg-canvas p-12 text-center">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-muted-soft"
            />
      ...
      >
        <div class="space-y-3">
          ...
          >
            ...
            <div class="text-right text-sm text-ink">
              <slot
  ...
  >
    <table class="w-full min-w-max divide-y divide-hairline">
      <thead class="table-header bg-surface-soft">
        <tr>
          <th
            ...
            :class="[
              ...
              { 'cursor-pointer hover:bg-surface-card ': column.sortable },
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-muted-soft">
                  <svg
      ...
      <tbody class="table-body divide-y divide-hairline bg-canvas">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          ...
          >
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-hairline"></div>
            </div>
          </td>
        </tr>
        ...
        <template v-else>
          ...
          >
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-ink ',
                getAdaptivePaddingClass(),
...
const columnsSignature = computed(() =>
  props.columns
    .map((column) => `${column.key}:${column.sortable ? "1" : "0"}`)
    .join("| "),
);
...
.is-scrollable .sticky-col-left::after {
  ...
  box-shadow: 8px 0 12px -12px rgba(20, 20, 19, 0.3);
  pointer-events: none;
}
...
.table-wrapper::-webkit-scrollbar-track {
  background-color: rgba(20, 20, 19, 0.03) !important;
...
.table-wrapper::-webkit-scrollbar-thumb {
  background-color: rgba(108, 106, 100, 0.75) !important;
  border-radius: 6px !important;
...
.table-wrapper::-webkit-scrollbar-thumb:hover {
  background-color: rgba(108, 106, 100, 0.9) !important;
}
...
@supports (-moz-appearance: none) {
  .table-wrapper {
    ...
    scrollbar-color: rgba(142, 139, 130, 0.5) rgba(20, 20, 19, 0.03) !important;
  }
}
</style>
