<template>
  <div class="space-y-4">
          ...
          <div v-if="paidOrder" class="w-full rounded-xl bg-cream p-4">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-ink">#{{ paidOrder.id }}</span>
              </div>
              <div v-if="paidOrder.out_trade_no" class="flex justify-between">
                <span class="text-muted">{{ t('payment.orders.orderNo') }}</span>
                <span class="font-medium text-ink">{{ paidOrder.out_trade_no }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-muted">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium text-ink">{{ paidOrder.order_type === 'balance' ? '$' + paidOrder.amount.toFixed(2) : formatGatewayAmount(paidOrder.amount) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-muted">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-ink">{{ formatGatewayAmount(paidOrder.pay_amount) }}</span>
              </div>
            </div>
          </div>
    ...
    <template v-else-if="outcome === 'cancelled'">
      <div class="card p-8">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="flex h-16 w-16 items-center justify-center rounded-full bg-surface-card">
            <svg class="h-8 w-8 text-muted-soft" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <p class="text-[22px] leading-tight text-ink">{{ t('payment.qr.cancelled') }}</p>
          <p class="text-sm text-body">{{ t('payment.qr.cancelledDesc') }}</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') }}</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="outcome === 'expired'">
      <div class="card p-8">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="flex h-16 w-16 items-center justify-center rounded-full bg-accent-amber/15">
            <svg class="h-8 w-8 text-accent-amber" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <p class="text-[22px] leading-tight text-ink">{{ t('payment.qr.expired') }}</p>
          <p class="text-sm text-body">{{ t('payment.qr.expiredDesc') }}</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') }}</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="qrUrl">
      <div class="card p-8">
        <div class="flex flex-col items-center space-y-4">
          <p class="text-[22px] leading-tight text-ink">{{ scanTitle }}</p>
          ...
          <p v-if="scanHint" class="text-center text-sm text-muted">{{ scanHint }}</p>
      ...
      <div class="card p-4 text-center">
        <p class="text-sm text-muted">{{ t('payment.qr.expiresIn') }}</p>
        <p class="mt-1 font-display text-[28px] leading-none tabular-nums text-ink">{{ countdownDisplay }}</p>
    ...
    <template v-else>
      <div class="card p-8">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-sm text-body">{{ t('payment.qr.payInNewWindowHint') }}</p>
          <button v-if="payUrl" class="btn btn-secondary text-sm" @click="reopenPopup">
            {{ t('payment.qr.openPayWindow') }}
          </button>
        </div>
...
const qrBorderClass = computed(() => {
  if (isAlipay.value) return 'border-accent-teal bg-surface-card'
  if (isWxpay.value) return 'border-success bg-surface-card'
  return 'border-hairline bg-canvas'
})

const qrLogoBgClass = computed(() => {
  if (isAlipay.value) return 'bg-accent-teal'
