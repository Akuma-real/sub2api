<template>
  <div
    ...
    :title="
      !enabled
        ? t('admin.settings.payment.typeDisabled') +
          ' — ' +
    ...
    >
      ...
      <div class="flex items-center gap-3">
        <div
          :class="[
            'rounded-md p-1.5',
            provider.enabled && enabled ? 'bg-success/15 ' : 'bg-surface-card ',
          ]"
        >
          <Icon
            name="server"
        ...
        <span class="text-xs text-muted-soft">{{ keyLabel }}</span>
        <span v-if="provider.payment_mode" class="text-xs text-muted-soft"
          >· {{ modeLabel }}</span
        >
        <span
          v-if="enabled && availableTypes.length"
...
const keyLabel = computed(() =>
  t(
    PROVIDER_KEY_LABELS[props.provider.provider_key] ||
      props.provider.provider_key,
  ),
);

const modeLabel = computed(() => {
  if (props.provider.payment_mode === PAYMENT_MODE_QRCODE)
