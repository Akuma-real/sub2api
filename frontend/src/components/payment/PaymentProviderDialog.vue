  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="border-t border-hairline pt-4">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            <template #trigger>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-hairline text-[11px] font-semibold text-muted-soft transition-colors hover:border-primary-500 hover:text-primary-600"
            ...
            <div class="space-y-3">
              ...
              <div
                ...
                :key="item.title"
                class="space-y-1.5 border-t border-on-dark/10 pt-2 first:border-t-0 first:pt-0"
              >
                <p class="font-medium text-on-primary">{{ item.title }}</p>
                <p>
                  <span class="text-muted-soft">{{
                    t("admin.settings.payment.guideOpenLabel")
                  }}</span
                  >{{ item.open }}
                </p>
                <p>
                  <span class="text-muted-soft">{{
                    t("admin.settings.payment.guideCallLabel")
                  }}</span
                  >{{ item.call }}
                </p>
                <p>
                  <span class="text-muted-soft">{{
                    t("admin.settings.payment.guideFallbackLabel")
                  }}</span
                  >{{ item.fallback }}
                </p>
              </div>
              <p
                v-if="paymentGuide.note"
                class="border-t border-on-dark/10 pt-2 text-[11px] text-muted-soft"
              >
                {{ paymentGuide.note }}
              </p>
            </div>
        ...
        <p v-if="paymentGuide" class="mb-3 text-xs text-muted">
          {{ paymentGuide.summary }}
        </p>
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-muted-soft"
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-muted-soft hover:text-body"
              >
                <svg
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              />
              <span
                class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-hairline bg-surface-soft px-3 text-xs text-muted"
        ...
        <!-- 服务商 Webhook 提示 -->
        <div
          v-if="providerWebhookUrl"
          class="mt-3 rounded-lg border border-accent-teal/30 bg-accent-teal/10 p-3"
        >
          <p class="text-xs text-primary-700">
            {{ t(providerWebhookHint) }}
          </p>
          <code
            class="mt-1 block break-all rounded bg-accent-teal/15 px-2 py-1 text-xs text-body-strong"
          >
            {{ providerWebhookUrl }}
          </code>
          <p
            v-if="form.provider_key === 'stripe'"
            class="mt-2 text-xs leading-relaxed text-primary-700"
          >
            {{
      ...
      <div v-if="limitableTypes.length" class="border-t border-hairline pt-4">
        <button
          type="button"
        ...
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          <div
            ...
            :key="lt.value"
            class="rounded-lg border border-hairline-soft p-3"
          >
            <p class="mb-2 text-xs font-medium text-body">{{ lt.label }}</p>
...
const providerWebhookUrl = computed(() => {
  ...
  return providerWebhookHintMap[form.provider_key] && path
    ? defaultBaseUrl + path
    : "";
});
...
const providerWebhookHint = computed(
  () =>
    providerWebhookHintMap[form.provider_key] ||
...
const supportsPaymentMode = computed(() =>
  providerSupportsPaymentMode(form.provider_key),
);
...
const paymentModeOptions = computed(() => {
  if (form.provider_key === "alipay") {
    ...
    return [
      ...
      {
        value: PAYMENT_MODE_REDIRECT,
        label: t("admin.settings.payment.modeRedirect"),
      },
    ];
  }
  return [
...
function loadProvider(provider: ProviderInstance) {
  ...
  form.payment_mode = isValidPaymentMode(
