<template>
  <div class="space-y-4">
    ...
    <div v-else-if="initError" class="card p-6 text-center">
      <p class="text-sm text-error">{{ initError }}</p>
      <button class="btn btn-secondary mt-4" @click="$emit('back')">
    ...
    <template v-else-if="success">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <div class="w-full rounded-xl bg-surface-soft p-4">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-muted">{{
                  t("payment.orders.orderId")
                }}</span>
                <span class="font-medium text-ink">#{{ orderId }}</span>
              </div>
    ...
    <template v-else>
      ...
      <div class="card overflow-hidden">
        <div
          class="bg-primary-500 px-6 py-5 text-center"
        >
...
const emit = defineEmits<{
  ...
  back: [];
  redirect: [orderId: number, payUrl: string];
}>();
...
async function handlePay() {
  ...
  if (POPUP_METHODS.has(selectedType.value)) {
    ...
    const onReady = (event: MessageEvent) => {
      if (event.source !== popup || event.data?.type !== "STRIPE_POPUP_READY")
        return;
