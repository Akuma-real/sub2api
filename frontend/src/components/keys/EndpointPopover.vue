...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="flex items-center gap-1.5 rounded-lg border border-hairline bg-canvas px-2.5 py-1.5 text-xs transition-colors hover:border-primary-200"
    >
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 rounded-xl border border-hairline bg-canvas px-3 py-2.5 text-left opacity-0 shadow-[0_14px_36px_-20px_rgba(20,20,19,0.18)] ring-1 ring-hairline/80 transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100"
        >
          <p
            v-if="item.description"
            class="max-w-[24rem] break-words text-xs leading-5 text-body"
          >
            {{ item.description }}
          </p>
          <p
          ...
          <div
            class="absolute left-1/2 top-full h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 border-b border-r border-hairline bg-canvas"
          ></div>
        </div>

        <code
          class="cursor-pointer font-mono text-muted decoration-muted-soft decoration-dashed underline-offset-2 hover:text-primary-600 hover:underline focus:text-primary-600 focus:underline focus:outline-none"
          role="button"
          tabindex="0"
        ...
        <button
          ...
          :class="
            copiedEndpoint === item.endpoint
              ? 'text-success '
              : 'text-muted-soft hover:text-primary-500 '
          "
        ...
        <a
          ...
          class="rounded p-0.5 text-muted-soft transition-colors hover:text-warning"
