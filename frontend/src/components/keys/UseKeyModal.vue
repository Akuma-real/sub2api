<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="!platform"
        class="flex items-start gap-3 p-4 rounded-lg bg-accent-amber/15 border border-accent-amber/30"
      >
        <svg
      ...
      <template v-else>
        ...
        <div class="space-y-4">
          ...
          >
            ...
            <p
              v-if="file.hint"
              class="text-xs text-warning mb-1.5 flex items-center gap-1"
            >
            ...
            <div class="bg-surface-dark rounded-xl overflow-hidden">
              <!-- Code Header -->
              <div
                class="flex items-center justify-between px-4 py-2 bg-surface-dark-soft border-b border-surface-dark-elevated"
              >
                ...
                <button
                  ...
                  :class="
                    copiedIndex === index
                      ...
                      : 'bg-surface-dark-elevated hover:bg-body text-muted-soft hover:text-on-primary'
                  "
                >
                  <svg
              ...
              <!-- Code Content -->
              <pre
                class="p-4 text-sm font-mono text-canvas overflow-x-auto"
        ...
        <div
          ...
          class="flex items-start gap-3 p-3 rounded-lg bg-accent-teal/10 border border-accent-teal/20"
        >
          <Icon
            name="infoCircle"
            size="md"
            class="text-accent-teal flex-shrink-0 mt-0.5"
          />
...
const string = (value: string) => wrapToken("text-warning", value);
...
$env:CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`;
  ...
  const vscodeContent = `{"env": {"ANTHROPIC_BASE_URL":"${baseUrl}","ANTHROPIC_AUTH_TOKEN":"${apiKey}","CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC":"1","CLAUDE_CODE_ATTRIBUTION_HEADER":"0"
  }
}`;
...
requires_openai_auth = true`;
  ...
  // auth.json content
  const authContent = `{"OPENAI_API_KEY":"${apiKey}"
}`;
