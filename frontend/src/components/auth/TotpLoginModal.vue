<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      <div class="fixed inset-0 bg-ink/50 transition-opacity"></div>

      <div class="relative w-full max-w-md transform rounded-lg border border-hairline bg-canvas p-6 shadow-card transition-all">
        <!-- Header -->
        <div class="mb-6 text-center">
          ...
          <h3 class="mt-4 text-[22px] leading-tight text-ink">
            {{ t('profile.totp.loginTitle') }}
          </h3>
          <p class="mt-2 text-sm text-muted">
            {{ t('profile.totp.loginHint') }}
          </p>
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium text-body">
            {{ userEmailMasked }}
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="flex justify-center gap-2">
            <input
              ...
              pattern="[0-9]"
              autocomplete="off"
              class="h-12 w-10 rounded-lg border border-hairline bg-canvas text-center text-lg font-semibold focus:border-primary-500 focus:ring-primary-500"
              :disabled="verifying"
          ...
          <!-- Loading indicator -->
          <div v-if="verifying" class="mt-3 flex items-center justify-center gap-2 text-sm text-muted">
