  ...
  >
    <div class="space-y-4">
      <div v-if="loading" class="flex items-center justify-center py-8">
        <svg
          class="h-6 w-6 animate-spin text-muted-soft"
          fill="none"
          viewBox="0 0 24 24"
        >
          <circle
      ...
      <div
        v-else-if="!isActive"
        class="rounded-lg border border-hairline p-4 text-sm text-muted"
      >
      ...
      <div v-else class="space-y-4">
        <div
          class="rounded-lg border border-success/25 bg-success/15 p-3 text-sm text-success"
        >
        ...
        <div class="rounded-lg border border-hairline p-3">
          ...
          <div class="mt-2 rounded bg-surface-soft p-2 text-xs text-body">
            {{ state?.error_message || "-" }}
          </div>
        </div>
      </div>
...
const remainingText = computed(() => {
  ...
  return t("admin.accounts.tempUnschedulable.remainingHoursMinutes", {
    hours,
    minutes: rest,
  });
});
...
const loadStatus = async () => {
  ...
  try {
    ...
    appStore.showError(
      error?.message || t("admin.accounts.tempUnschedulable.failedToLoad"),
    );
    status.value = null;
  } finally {
    loading.value = false;
