<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-muted-soft">{{ rateLimitResumeText }}</span>
    </div>

    <!-- Overload Display (529) - Two-line layout -->
    <div v-else-if="isOverloaded" class="flex flex-col items-center gap-1">
      <span class="badge text-xs badge-danger">{{
        t("admin.accounts.status.overloaded")
      }}</span>
      <span class="text-[11px] text-muted-soft">{{ overloadCountdown }}</span>
    </div>
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      <svg
        class="h-4 w-4 cursor-help text-error transition-colors hover:text-error"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
      ...
      <!-- Tooltip - 向下显示 -->
      <div
        class="invisible absolute left-0 top-full z-[100] mt-1.5 min-w-[200px] max-w-[300px] rounded-lg bg-surface-dark-soft px-3 py-2 text-xs text-on-primary opacity-0 shadow-card transition-all duration-200 group-hover/error:visible group-hover/error:opacity-100"
      >
        <div
          class="whitespace-pre-wrap break-words leading-relaxed text-muted-soft"
        >
          {{ account.error_message }}
        </div>
        <!-- 上方小三角 -->
        <div
          class="absolute bottom-full left-3 border-[6px] border-transparent border-b-surface-dark-soft"
        ></div>
      </div>
    </div>
    ...
    <div v-if="isRateLimited" class="group relative">
      <span
        class="inline-flex items-center gap-1 rounded bg-accent-amber/15 px-1.5 py-0.5 text-xs font-medium text-warning"
      >
      ...
      <!-- Tooltip -->
      <div
        class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 whitespace-normal rounded bg-surface-dark px-3 py-2 text-center text-xs leading-relaxed text-on-primary opacity-0 transition-opacity group-hover:opacity-100"
      >
        {{
    ...
    >
      ...
      >
        ...
        <span
          ...
          class="inline-flex items-center gap-1 rounded bg-error/15 px-1.5 py-0.5 text-xs font-medium text-error"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        ...
        <span
          v-else
          class="inline-flex items-center gap-1 rounded bg-primary-100 px-1.5 py-0.5 text-xs font-medium text-primary-700"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ formatScopeName(item.model) }}
          <span class="text-[10px] opacity-70">{{
  ...
  for (const [model, info] of Object.entries(modelLimits)) {
    ...
    if (model === "AICredits") {
      ...
      items.push({
        ...
        model,
        reset_at: info.rate_limit_reset_at,
      });
...
const rateLimitResumeText = computed(() => {
  ...
  return t("admin.accounts.status.rateLimitedAutoResume", {
    time: rateLimitCountdown.value,
  });
});
