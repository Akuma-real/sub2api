<template>
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    >
      ...
      <div v-if="loading" class="space-y-1.5">
        ...
        <div class="flex items-center gap-1">
          <div class="h-3 w-[32px] animate-pulse rounded bg-hairline-soft"></div>
          <div class="h-1.5 w-8 animate-pulse rounded-full bg-hairline-soft"></div>
          <div class="h-3 w-[32px] animate-pulse rounded bg-hairline-soft"></div>
        </div>
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div v-if="usageInfo.error" class="text-xs text-accent-amber" :title="usageInfo.error">
          {{ usageInfo.error }}
        </div>
        <!-- 5h Window -->
        <UsageProgressBar
        ...
        <div class="flex items-center gap-1.5 mt-0.5">
          <span
            ...
            class="text-[9px] text-muted-soft"
          >
            {{ t('admin.accounts.usageWindow.passiveSampled') }}
          </span>
          <button
            type="button"
            class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-medium text-primary-600 hover:bg-surface-card"
      ...
      <!-- No data yet -->
      <div v-else class="text-xs text-muted-soft">-</div>
    </template>
    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          ...
          <span
            class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-surface-dark px-3 py-2 text-xs leading-relaxed text-on-dark opacity-0 shadow-lg transition-opacity group-hover:opacity-100"
          >
      ...
      <div v-if="isForbidden" class="space-y-1">
        ...
        <div v-if="validationURL" class="flex items-center gap-1">
          <a
            ...
            class="text-[10px] text-primary-600 hover:text-primary-700 hover:underline"
          ...
          <button
            type="button"
            class="text-[10px] text-muted hover:text-body"
      ...
      <div v-else-if="needsReauth" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-accent-amber/15 text-primary-600">
      ...
      <div v-else-if="usageInfo?.error" class="space-y-1">
        <span class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-accent-amber/15 text-warning">
          {{ usageErrorLabel }}
        </span>
      </div>
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        >
          <svg
            class="h-3.5 w-3.5 text-muted-soft hover:text-body"
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
          ...
          >
            ...
            <div class="mb-2 text-on-dark-soft">{{ t('admin.accounts.gemini.quotaPolicy.note') }}</div>
            <div class="space-y-1">
              <div><strong>{{ geminiQuotaPolicyChannel }}:</strong></div>
              <div class="pl-2">• {{ geminiQuotaPolicyLimits }}</div>
              <div class="mt-2">
                <a :href="geminiQuotaPolicyDocsUrl" target="_blank" rel="noopener noreferrer" class="text-primary-600 underline hover:text-primary-700">
      ...
      <div class="space-y-1">
        ...
        >
          <div class="flex items-center gap-1.5 text-[9px] text-muted">
            <span class="rounded bg-surface-card px-1.5 py-0.5">
              {{ formatKeyRequests }} req
            </span>
            <span class="rounded bg-surface-card px-1.5 py-0.5">
              {{ formatKeyTokens }}
            </span>
            <span class="rounded bg-surface-card px-1.5 py-0.5" :title="t('usage.accountBilled')">
              A ${{ formatKeyCost }}
            </span>
            <span
              v-if="todayStats.user_cost != null"
              class="rounded bg-surface-card px-1.5 py-0.5"
        ...
        >
          <div class="h-3 w-10 animate-pulse rounded bg-hairline-soft"></div>
          <div class="h-3 w-8 animate-pulse rounded bg-hairline-soft"></div>
          <div class="h-3 w-12 animate-pulse rounded bg-hairline-soft"></div>
        </div>
        ...
        <div v-else-if="geminiUsageAvailable" class="space-y-1">
          <UsageProgressBar
            ...
            :color="bar.color"
          />
          <p class="mt-1 text-[9px] leading-tight text-muted-soft">
            * {{ t('admin.accounts.gemini.quotaPolicy.simulatedNote') || 'Simulated quota' }}
          </p>
        </div>
        <!-- AI Studio Client OAuth: show unlimited flow (no usage tracking) -->
        <div v-else class="text-xs text-muted-soft">
  ...
  <div ref="rootRef" v-else>
    ...
    <div v-else class="space-y-1">
      ...
      <!-- No data at all -->
      <div v-if="!todayStats && !todayStatsLoading && !hasApiKeyQuota" class="text-xs text-muted-soft">-</div>
    </div>
  </div>
</template>
...
const geminiTierClass = computed(() => {
  ...
  if (channel === 'client' || channel === 'ai studio') {
        return 'bg-surface-card text-primary-600'
  }

  if (channel === 'google one') {
    if (level === 'ultra') return 'bg-surface-card text-accent-amber'
    if (level === 'pro') return 'bg-surface-card text-primary-600'
    return 'bg-surface-card text-body'
  }

  if (channel === 'gcp') {
    if (level === 'enterprise') return 'bg-surface-card text-accent-amber'
    return 'bg-surface-card text-primary-600'
  }
...
const antigravityTierClass = computed(() => {
  switch (antigravityTier.value) {
    ...
    case 'g1-pro-tier':
      return 'bg-surface-card text-primary-600'
    case 'g1-ultra-tier':
      return 'bg-surface-card text-accent-amber'
    default:
      return ''
  }
})
