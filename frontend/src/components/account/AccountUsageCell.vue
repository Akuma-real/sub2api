  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    >
      ...
      <div v-if="loading" class="space-y-1.5">
        ...
        <div class="flex items-center gap-1">
          <div class="h-3 w-[32px] animate-pulse rounded bg-hairline"></div>
          <div class="h-1.5 w-8 animate-pulse rounded-full bg-hairline"></div>
          <div class="h-3 w-[32px] animate-pulse rounded bg-hairline"></div>
        </div>
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div
          ...
          class="text-xs text-warning truncate max-w-[200px]"
        ...
        <div class="flex items-center gap-1.5 mt-0.5">
          <span
            ...
            class="text-[9px] text-muted-soft italic"
          >
            {{ t("admin.accounts.usageWindow.passiveSampled") }}
          </span>
          <button
            type="button"
            class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-medium text-primary-700 hover:bg-accent-teal/10 transition-colors"
    ...
    >
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        <span v-if="hasIneligibleTiers" class="group relative cursor-help">
          ...
          <span
            class="pointer-events-none absolute left-0 top-full z-50 mt-1 w-80 whitespace-normal break-words rounded bg-surface-dark px-3 py-2 text-xs leading-relaxed text-on-primary opacity-0 shadow-card transition-opacity group-hover:opacity-100"
          >
      ...
      <div v-if="isForbidden" class="space-y-1">
        ...
        <div v-if="validationURL" class="flex items-center gap-1">
          <a
            ...
            class="text-[10px] text-primary-700 hover:text-body-strong hover:underline"
      ...
      <div v-else-if="needsReauth" class="space-y-1">
        <span
          class="inline-block rounded px-1.5 py-0.5 text-[10px] font-medium bg-accent-amber/15 text-warning"
        >
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        <span class="group relative cursor-help">
          ...
          >
            ...
            <div class="mb-2 text-muted-soft">
            ...
            <div class="space-y-1">
              ...
              <div class="mt-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="text-accent-teal hover:text-accent-teal underline"
                >
      ...
      <div class="space-y-1">
        ...
        >
          <div class="h-3 w-10 animate-pulse rounded bg-hairline"></div>
          <div class="h-3 w-8 animate-pulse rounded bg-hairline"></div>
          <div class="h-3 w-12 animate-pulse rounded bg-hairline"></div>
        </div>
        ...
        <div v-else-if="geminiUsageAvailable" class="space-y-1">
          <UsageProgressBar
            ...
            :color="bar.color"
          />
          <p class="mt-1 text-[9px] leading-tight text-muted-soft italic">
            *
            {{
...
let desktopViewportListener: ((event: MediaQueryListEvent) => void) | null =
  null;
...
const geminiTierClass = computed(() => {
  ...
  if (channel === "google one") {
    if (level === "ultra") return "bg-primary-100 text-primary-700 ";
    if (level === "pro") return "bg-accent-teal/15 text-primary-700 ";
    return "bg-surface-card text-body ";
  }

  if (channel === "gcp") {
    if (level === "enterprise") return "bg-primary-100 text-primary-700 ";
    return "bg-accent-teal/15 text-primary-700 ";
  }
...
const forbiddenType = computed(
  () => usageInfo.value?.forbidden_type || "forbidden",
);
...
const quotaDailyBar = computed((): QuotaBarInfo | null => {
  ...
  return makeQuotaBar(
    props.account.quota_daily_used ?? 0,
    limit,
    "quota_daily_start",
  );
});
...
const quotaWeeklyBar = computed((): QuotaBarInfo | null => {
  ...
  if (limit <= 0) return null;
  return makeQuotaBar(
    props.account.quota_weekly_used ?? 0,
    limit,
    "quota_weekly_start",
  );
});
...
const formatKeyRequests = computed(() => {
  ...
  return formatCompactNumber(props.todayStats.requests, {
    allowBillions: false,
  });
});
...
onMounted(() => {
  if (typeof window !== "undefined") {
    ...
    if (typeof desktopViewportMediaQuery.addEventListener === "function") {
      desktopViewportMediaQuery.addEventListener(
        "change",
...
onUnmounted(() => {
  ...
  if (desktopViewportMediaQuery && desktopViewportListener) {
    if (typeof desktopViewportMediaQuery.removeEventListener === "function") {
      desktopViewportMediaQuery.removeEventListener(
        "change",
        desktopViewportListener,
      );
    } else {
