  ...
  <div>
    ...
    <div class="relative mb-3">
      <div
        ...
        class="cursor-pointer rounded-lg border border-hairline bg-canvas px-3 py-2"
      >
        <div class="grid grid-cols-2 gap-1.5">
          <span
            v-for="model in modelValue"
            :key="model"
            class="inline-flex items-center justify-between gap-1 rounded bg-surface-card px-2 py-1 text-xs text-body"
          >
            ...
            <button
              ...
              class="shrink-0 rounded-full hover:bg-hairline-soft"
            >
        ...
        <div class="mt-2 flex items-center justify-between border-t border-hairline pt-2">
          <span class="text-xs text-muted-soft">{{ t('admin.accounts.modelCount', { count: modelValue.length }) }}</span>
          <svg class="h-5 w-5 text-muted-soft" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div
        v-if="showDropdown"
        class="absolute left-0 right-0 top-full z-50 mt-1 rounded-lg border border-hairline bg-canvas shadow-lg"
      >
        <div class="sticky top-0 border-b border-hairline bg-canvas p-2">
          <input
            v-model="searchQuery"
            type="text"
        ...
        <div class="max-h-52 overflow-auto">
          <button
            ...
            class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-surface-card"
          >
            <span
              :class="[
            ...
            <span class="truncate text-ink">{{ model.value }}</span>
          </button>
          <div v-if="filteredModels.length === 0" class="px-3 py-4 text-center text-sm text-muted">
    ...
    <div class="mb-4 flex flex-wrap gap-2">
      <button
        type="button"
        @click="fillRelated"
        class="rounded-lg border border-hairline px-3 py-1.5 text-sm text-primary-600 hover:bg-surface-card"
      >
      ...
      <button
        type="button"
        @click="clearAll"
        class="rounded-lg border border-error/25 px-3 py-1.5 text-sm text-error hover:bg-error/15"
      >
    ...
    <!-- Custom Model Input -->
    <div class="mb-3">
      <label class="mb-1.5 block text-sm font-medium text-body">{{ t('admin.accounts.customModelName') }}</label>
