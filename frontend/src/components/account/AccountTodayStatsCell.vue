<template>
  <div>
    ...
    <div v-if="props.loading && !props.stats" class="space-y-0.5">
      ...
      <div class="h-3 w-16 animate-pulse rounded bg-hairline"></div>
      <div class="h-3 w-10 animate-pulse rounded bg-hairline"></div>
    </div>

    <!-- Error state -->
    <div v-else-if="props.error && !props.stats" class="text-xs text-error">
      {{ props.error }}
    </div>
    ...
    <div v-else-if="props.stats" class="space-y-0.5 text-xs">
      ...
      <!-- Tokens -->
      <div class="flex items-center gap-1">
        <span class="text-muted">{{ t("admin.accounts.stats.tokens") }}:</span>
        <span class="font-medium text-body">{{
