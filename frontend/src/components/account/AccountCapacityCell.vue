...
const concurrencyClass = computed(() => {
  ...
  if (current >= max) return "bg-error/15 text-error ";
  if (current > 0) return "bg-accent-amber/15 text-warning ";
...
const windowCostClass = computed(() => {
  ...
  if (current >= limit + reserve) return "bg-error/15 text-error ";
  if (current >= limit) return "bg-accent-amber/15 text-warning ";
  if (current >= limit * 0.8) return "bg-accent-amber/15 text-warning ";
...
const sessionLimitClass = computed(() => {
  ...
  if (current >= max) return "bg-error/15 text-error ";
  if (current >= max * 0.8) return "bg-accent-amber/15 text-warning ";
  return "bg-success/15 text-success ";
});
...
const rpmStrategyTag = computed(() =>
  rpmStrategy.value === "sticky_exempt" ? "[S]" : "[T]",
);

const rpmBuffer = computed(() => {
  const base = props.account.base_rpm || 0;
  return (
    props.account.rpm_sticky_buffer ??
    (base > 0 ? Math.max(1, Math.floor(base / 5)) : 0)
  );
});
...
const rpmClass = computed(() => {
  ...
  if (rpmStrategy.value === "tiered") {
    if (current >= base + buffer) return "bg-error/15 text-error ";
    if (current >= base) return "bg-accent-amber/15 text-warning ";
  } else {
    if (current >= base) return "bg-accent-amber/15 text-warning ";
  }
  if (current >= base * 0.8) return "bg-accent-amber/15 text-warning ";
  return "bg-success/15 text-success ";
});
...
const showDailyQuota = computed(
  () =>
    isQuotaEligible.value &&
    props.account.quota_daily_limit != null &&
    props.account.quota_daily_limit > 0,
);
const showWeeklyQuota = computed(
  () =>
    isQuotaEligible.value &&
    props.account.quota_weekly_limit != null &&
    props.account.quota_weekly_limit > 0,
);
