<template>
  ...
  >
    ...
    >
      ...
      <div class="rounded-lg bg-surface-soft p-3 text-xs text-muted">
      ...
      <div class="grid grid-cols-1 gap-4">
        <div>
          <label for="crs-base-url" class="input-label">{{
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label for="crs-username" class="input-label">{{
          ...
          <div>
            <label for="crs-password" class="input-label">{{
    ...
    <div
      v-else-if="currentStep === 'preview' && previewResult"
      class="space-y-4"
    >
      ...
      >
        <div class="mb-2 text-sm font-medium text-body">
          ...
          <span class="ml-1 text-xs text-muted-soft"
            >({{ previewResult.existing_accounts.length }})</span
          >
        </div>
        <div class="max-h-32 overflow-auto text-xs text-muted">
          <div
          ...
          >
            <span
              class="inline-block rounded bg-accent-teal/15 px-1.5 py-0.5 text-[10px] font-medium text-primary-700"
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-ink">
            ...
            <span class="ml-1 text-xs text-muted-soft"
              >({{ previewResult.new_accounts.length }})</span
            >
          </div>
          <div class="flex gap-2">
            <button
              type="button"
              class="text-xs text-primary-700 hover:text-primary-700"
              @click="selectAll"
            >
        ...
        <div
          class="max-h-48 overflow-auto rounded-lg border border-hairline p-2"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-surface-soft"
          >
            <input
              type="checkbox"
            ...
            />
            <span
              class="inline-block rounded bg-success/15 px-1.5 py-0.5 text-[10px] font-medium text-success"
              >{{ acc.platform }} / {{ acc.type }}</span
            >
            <span class="truncate text-sm text-body">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-muted">
        ...
        <span
          :class="form.sync_proxies ? 'text-success ' : 'text-muted-soft '"
        >
      ...
      <div
        ...
        class="rounded-lg bg-surface-soft p-4 text-center text-sm text-muted"
      >
        {{ t("admin.accounts.crsNoNewAccounts") }}
        <span v-if="previewResult.existing_accounts.length">
          {{
            t("admin.accounts.crsWillUpdate", {
              count: previewResult.existing_accounts.length,
            })
          }}
        </span>
      </div>
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      <div class="space-y-2 rounded-xl border border-hairline p-4">
        <div class="text-sm font-medium text-ink">
      ...
      <div class="flex justify-end gap-3">
        ...
        <template v-if="currentStep === 'input'">
          ...
          >
            {{
              previewing
                ? t("admin.accounts.crsPreviewing")
                : t("admin.accounts.crsPreview")
            }}
          </button>
        </template>
        ...
        <template v-else-if="currentStep === 'preview'">
          ...
          >
            {{
              syncing
                ? t("admin.accounts.syncing")
                : t("admin.accounts.syncNow")
            }}
          </button>
        </template>
...
const hasNewButNoneSelected = computed(() => {
  ...
  return (
    previewResult.value.new_accounts.length > 0 && selectedIds.value.size === 0
  );
});
...
const errorItems = computed(() => {
  ...
  return result.value.items.filter(
    (i) =>
      i.action === "failed" ||
      (i.action === "skipped" && i.error !== "not selected"),
  );
});
...
const selectAll = () => {
  ...
  selectedIds.value = new Set(
    previewResult.value.new_accounts.map((a) => a.crs_account_id),
  );
};
