  ...
  <div v-if="shouldShowQuota">
    ...
    <div class="text-xs text-muted-soft">
      ...
      <span
        ...
        :class="[
          'font-medium',
          isUrgent ? 'text-error animate-pulse' : 'text-warning ',
        ]"
      >
        {{
...
const tierBadgeClass = computed(() => {
  ...
  if (isCodeAssist.value) {
    const tier = (creds?.tier_id || "").toString().trim().toLowerCase();
    if (tier === "gcp_enterprise") return "bg-primary-100 text-primary-700 ";
    if (tier === "gcp_standard") return "bg-accent-teal/15 text-primary-700 ";
    // Backward compatibility
    const upper = (creds?.tier_id || "").toString().trim().toUpperCase();
    if (upper.includes("ULTRA") || upper.includes("ENTERPRISE"))
  ...
  if (isGoogleOne.value) {
    const tier = (creds?.tier_id || "").toString().trim().toLowerCase();
    if (tier === "google_ai_ultra") return "bg-primary-100 text-primary-700 ";
    if (tier === "google_ai_pro") return "bg-accent-teal/15 text-primary-700 ";
    if (tier === "google_one_free") return "bg-surface-card text-body ";
    // Backward compatibility
    const upper = (creds?.tier_id || "").toString().trim().toUpperCase();
    if (upper === "GOOGLE_ONE_UNLIMITED")
      return "bg-primary-100 text-primary-700 ";
    if (upper === "AI_PREMIUM") return "bg-accent-teal/15 text-primary-700 ";
    return "bg-surface-card text-body ";
  }

  // AI Studio 默认样式：蓝色
  const tier = (creds?.tier_id || "").toString().trim().toLowerCase();
  if (tier === "aistudio_paid") return "bg-accent-teal/15 text-primary-700 ";
  if (tier === "aistudio_free") return "bg-surface-card text-body ";
  return "bg-accent-teal/15 text-primary-700 ";
});
