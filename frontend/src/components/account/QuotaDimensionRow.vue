...
const resetDayOptions = computed(() =>
  props.dayOptions.map((d) => ({
    value: d.value,
    label: t("admin.accounts.dayOfWeek." + d.key),
  })),
);
const resetHourOptions = computed(() =>
  props.hourOptions.map((hour) => ({
    value: hour,
...
const onModeChange = (val: QuotaResetMode | null) => {
  if (!val) return;
  emit("update:resetMode", val);
  if (val === "fixed") {
    if (props.resetHour == null) emit("update:resetHour", 0);
    if (props.dim === "weekly" && props.resetDay == null)
      emit("update:resetDay", 1);
...
<template>
  <div>
    ...
    <div v-if="quotaNotifyGlobalEnabled" class="flex items-center gap-2 mb-1">
      <span class="text-xs font-medium text-body flex-1 min-w-0">{{
        label
      }}</span>
      <span
        v-if="limit && limit > 0"
        class="text-xs font-medium text-body flex-1 min-w-0"
        >{{ t("admin.accounts.quotaNotify.alert") }}</span
      >
    </div>
    <label v-else class="text-xs font-medium text-body mb-1 block">{{
      label
    }}</label>
    ...
    <div class="flex items-center gap-2">
      <div
        :class="[
          'relative',
          quotaNotifyGlobalEnabled ? 'flex-1 min-w-0' : 'flex-1',
        ]"
      >
        <span
          class="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted text-sm"
          >$</span
        >
        <input
          :value="limit"
          ...
          min="0"
          step="0.01"
          class="input pl-6 py-1.5 text-sm"
    ...
    <div v-if="hasResetMode" class="mt-1 flex items-center gap-2 flex-wrap">
      ...
      <Select
        ...
        class="w-auto"
        @update:model-value="onModeChange($event as QuotaResetMode | null)"
      />
    ...
    <p v-if="!hasResetMode" class="input-hint mb-0 text-[11px]">
      {{ hintRolling }}
    </p>
  </div>
</template>
