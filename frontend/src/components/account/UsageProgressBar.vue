<template>
  <div>
    ...
    <div class="flex items-center gap-1">
      ...
      <div class="h-1.5 w-8 shrink-0 overflow-hidden rounded-full bg-hairline">
        <div
      ...
      <span
        ...
        class="shrink-0 text-[10px] text-muted-soft"
      >
        {{ formatResetTime }}
      </span>
    </div>
...
const labelClass = computed(() => {
  const colors = {
    ...
    amber: "bg-accent-amber/15 text-warning ",
  };
...
const formatResetTime = computed(() => {
  ...
  // 显示「待刷新」以区别于真正可用的「现在」。
  if (diffMs <= 0) {
    return props.utilization > 0
      ? t("usage.resetPending")
      : t("usage.resetNow");
  }
...
const formatRequests = computed(() => {
  ...
  return formatCompactNumber(props.windowStats.requests, {
    allowBillions: false,
  });
});
