<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg
          class="mx-auto h-12 w-12 text-muted-soft"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          stroke-width="1"
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-hairline bg-canvas p-3"
        >
          <!-- Drag Handle -->
          <div
            class="cursor-move text-muted-soft hover:text-body"
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-ink">{{ attr.name }}</span>
              <span
                class="rounded bg-surface-card px-1.5 py-0.5 font-mono text-xs text-muted"
              >
                {{ attr.key }}
              </span>
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-muted hover:bg-surface-card hover:text-primary-600"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-muted hover:bg-error/15 hover:text-error"
  ...
  >
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      >
        <label class="input-label">{{
          t("admin.users.attributes.options")
        }}</label>
        <div
      ...
      <div>
        <label class="input-label">{{
          t("admin.users.attributes.fieldDescription")
        }}</label>
        <input
      ...
      <div>
        <label class="input-label">{{
          t("admin.users.attributes.placeholder")
        }}</label>
        <input
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input
            ...
            type="checkbox"
            class="h-4 w-4 rounded border-hairline text-primary-600"
          />
  ...
  <ConfirmDialog
    ...
    :message="
      t('admin.users.attributes.deleteConfirm', {
        name: deletingAttribute?.name,
      })
    "
...
const getOptionKey =
  createStableObjectKeyResolver<UserAttributeOption>("user-attr-option");
...
const loadAttributes = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.users.attributes.failedToLoad"),
    );
  } finally {
    loading.value = false;
  }
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail ||
        t("admin.users.attributes.failedToDelete"),
    );
  }
};
