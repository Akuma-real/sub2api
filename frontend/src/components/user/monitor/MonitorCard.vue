<template>
  <button
    type="button"
    class="group text-left p-5 rounded-lg min-h-[280px] w-full bg-canvas/70  border border-hairline/80 shadow-card hover:-translate-y-1 hover:shadow-card-hover hover:border-hairline transition-all duration-300 ease-out flex flex-col"
    @click="emit('click')"
  >
    <!-- Header: icon + name/model + status chip -->
    <div class="flex items-start gap-3">
      <span
        class="w-9 h-9 rounded-xl ring-1 ring-ink/5 grid place-items-center flex-shrink-0"
        :class="[providerGradient(item.provider), providerTintClass]"
      >
        <ProviderIcon :provider="item.provider" :size="20" />
      </span>
      <div class="flex-1 min-w-0">
        <div class="text-base font-semibold truncate text-ink">
          {{ item.name }}
        </div>
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          <span class="font-mono text-xs truncate text-muted">
            {{ item.primary_model }}
          </span>
          <span
            v-if="item.group_name"
            class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-surface-card text-body flex-shrink-0"
          >
            {{ item.group_name }}
          </span>
        </div>
    ...
    <!-- Divider -->
    <div class="mt-4 border-t border-hairline-soft"></div>
...
const providerTintClass = computed(
  () => PROVIDER_TINT[props.item.provider] ?? "text-muted ",
);
