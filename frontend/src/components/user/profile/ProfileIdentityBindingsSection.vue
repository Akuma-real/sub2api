<template>
  <div :class="props.embedded ? 'space-y-4' : 'card overflow-hidden'">
    ...
    <div :class="props.embedded ? 'space-y-4' : 'divide-y divide-hairline-soft'">
...
const rowClass = computed(() =>
  props.embedded
    ? compact.value
      ? 'rounded-lg border border-hairline bg-canvas p-4'
      : 'rounded-lg border border-hairline bg-surface-card p-4'
    : 'px-6 py-5'
)
...
function providerIconClass(provider: UserAuthProvider): string {
  ...
  if (provider === 'wechat') {
    return 'bg-surface-card text-success'
  }
  if (provider === 'oidc') {
    return 'bg-surface-card text-accent-teal'
  }
  return 'bg-surface-card text-primary-600'
}
