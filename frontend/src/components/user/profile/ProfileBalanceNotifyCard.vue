    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <label class="relative inline-flex items-center cursor-pointer">
          ...
          />
          <div
            class="w-11 h-6 bg-hairline peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-canvas after:border-hairline after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-600"
          ></div>
        </label>
      </div>
      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="text-xs text-muted-soft ml-2">{{
        ...
        <div>
          ...
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div
              ...
              :key="idx"
              class="flex items-center justify-between px-3 py-2 bg-surface-soft rounded-lg"
            >
                ...
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    <input
                      ...
                      type="text"
                      maxlength="6"
                      class="w-20 rounded border border-hairline px-2 py-1 text-xs"
                    ...
                    <button
                      @click="verifyingEmail = ''"
                      class="text-xs text-muted-soft hover:text-body"
                    >
                      {{ t("common.cancel") }}
                    </button>
                  </template>
          ...
          <div v-if="pendingEmails.length > 0" class="space-y-2 mb-3">
            <div
              ...
              :key="pe.email"
              class="flex items-center gap-2 px-3 py-2 bg-accent-amber/15 rounded-lg border border-accent-amber/30"
            >
              <span class="flex-1 text-sm text-body">{{ pe.email }}</span>
              <div v-if="!pe.codeSent" class="flex items-center gap-1">
                ...
                <button
                  ...
                  class="text-xs text-error hover:text-error ml-1"
                >
                  {{ t("profile.balanceNotify.removeEmail") }}
                </button>
              </div>
              <div v-else class="flex items-center gap-1">
                ...
                <span v-if="pe.countdown > 0" class="text-xs text-muted-soft"
                  >{{ pe.countdown }}s</span
                >
                <button
                  v-else
          ...
          <p v-else class="text-xs text-muted-soft">
...
const canAddMore = computed(() => {
  return (
    emailEntries.value.length + pendingEmails.value.length < maxTotalEmails
  );
});
...
const handleThresholdUpdate = async () => {
  ...
  try {
    const threshold =
      customThreshold.value && customThreshold.value > 0
...
function addPendingEmail() {
  ...
  // Check duplicates
  const isDuplicate =
    emailEntries.value.some(
      (e) => e.email.toLowerCase() === email.toLowerCase(),
    ) ||
    pendingEmails.value.some(
