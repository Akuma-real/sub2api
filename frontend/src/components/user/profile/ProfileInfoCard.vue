<template>
  <div class="space-y-6">
    <section
      ...
      class="card overflow-hidden border border-primary-100/80 bg-surface-soft"
    >
      <div class="px-6 py-6 md:px-8">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
          <div
            class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-primary-500 text-2xl font-bold text-on-primary shadow-card shadow-primary-500/20"
          >
            <img
              v-if="avatarUrl"
              :src="avatarUrl"
          ...
          <div class="min-w-0 flex-1 space-y-5">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                <h2 class="truncate text-2xl font-semibold text-ink">
                  {{ displayName }}
                </h2>
                <span
                  :class="[
              ...
              <div class="space-y-1">
                ...
                >
                  <span
                    ...
                    :key="hint.key"
                    class="inline-flex items-center gap-1 rounded-full bg-canvas/80 px-3 py-1 ring-1 ring-primary-100"
                  >
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              <div
                ...
                class="rounded-lg bg-canvas/85 px-4 py-3 shadow-sm ring-1 ring-hairline/70"
              >
                <p
                  class="text-xs font-medium uppercase tracking-[0.16em] text-muted-soft"
                >
    ...
    <div class="space-y-6">
      <div data-testid="profile-main-column" class="space-y-6">
        <section
          ...
          class="card border border-hairline-soft bg-canvas/90 p-6"
        >
          ...
          <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
            <div
              class="rounded-lg border border-hairline-soft bg-surface-soft/80 p-5"
            >
        ...
        >
          ...
          <div class="mt-5 grid gap-3">
            <div
              v-for="hint in sourceHints"
              :key="hint.key"
              class="flex items-start gap-3 rounded-lg border border-hairline-soft bg-surface-soft/80 px-4 py-3 text-sm text-body"
            >
              <Icon name="link" size="sm" class="mt-0.5 text-muted-soft" />
              <span>{{ hint.text }}</span>
            </div>
          </div>
        </section>
...
const sourceHints = computed(() => {
  ...
  if (avatarSource) {
    hints.push({
      key: "avatar",
      text: t("profile.authBindings.source.avatar", {
        providerName: avatarSource.label,
      }),
    });
  }
  ...
  if (usernameSource) {
    hints.push({
      key: "username",
      text: t("profile.authBindings.source.username", {
        providerName: usernameSource.label,
      }),
    });
  }
