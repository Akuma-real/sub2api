<template>
  <div class="fixed inset-0 z-50 overflow-y-auto" @click.self="$emit('close')">
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div
        class="relative w-full max-w-md transform rounded-xl bg-canvas p-6 shadow-card transition-all"
      >
        <!-- Header -->
        ...
        <div v-if="step === 1" class="space-y-6">
          ...
          <template v-if="setupData">
            <div class="flex justify-center">
              <div class="rounded-lg border border-hairline p-4 bg-canvas">
            ...
            <div class="text-center">
              ...
              <div class="flex items-center justify-center gap-2">
                <code
                  class="rounded bg-surface-card px-3 py-2 font-mono text-sm"
                >
                  {{ setupData.secret }}
                </code>
                <button
                  type="button"
                  class="rounded p-1.5 text-muted hover:bg-surface-card"
                  @click="copySecret"
                >
                  <svg
                    class="h-5 w-5"
            ...
            <div class="mb-6">
              ...
              <div class="flex justify-center gap-2">
                <input
                  ...
                  inputmode="numeric"
                  pattern="[0-9]"
                  class="h-12 w-10 rounded-lg border border-hairline text-center text-lg font-semibold focus:border-primary-500 focus:ring-primary-500"
...
const handleSendCode = async () => {
  ...
  try {
    ...
    appStore.showError(
      err.response?.data?.message || t("profile.totp.sendCodeFailed"),
    );
  } finally {
...
const handleVerifyAndSetup = async () => {
  ...
  try {
    ...
    appStore.showError(
      err.response?.data?.message || t("profile.totp.setupFailed"),
    );
  } finally {
...
const handleVerify = async () => {
  ...
  try {
    ...
    appStore.showError(
      err.response?.data?.message || t("profile.totp.verifyFailed"),
    );
    code.value = ["", "", "", "", "", ""];
    nextTick(() => {
