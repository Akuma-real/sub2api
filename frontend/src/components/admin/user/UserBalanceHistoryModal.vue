  ...
  >
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-xl bg-surface-soft p-4">
        ...
        <div
          class="mt-2.5 flex items-center justify-between border-t border-hairline/60 pt-2.5"
        >
          <p
            class="min-w-0 flex-1 truncate text-xs text-muted"
            :title="user.notes || ''"
          >
            <template v-if="user.notes"
              >{{ t("admin.users.notes") }}: {{ user.notes }}</template
            >
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-muted">
            {{ t("admin.users.totalRecharged") }}:
            <span class="font-semibold text-success"
              >${{ totalRecharged.toFixed(2) }}</span
            >
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="!hideActions"
          @click="emit('deposit')"
          class="flex items-center gap-2 rounded-lg border border-hairline bg-canvas px-3 py-2 text-sm text-body transition-colors hover:bg-surface-soft"
        >
          <Icon
            name="plus"
            size="sm"
        ...
        >
          <div class="flex items-start justify-between">
            ...
            <div class="text-right">
              ...
              <p v-if="isAdminType(item.type)" class="text-xs text-muted-soft">
                {{ t("redeem.adminAdjustment") }}
              </p>
              <p v-else class="font-mono text-xs text-muted-soft">
                {{ item.code.slice(0, 8) }}...
              </p>
            </div>
          </div>
...
const getIconBg = (item: BalanceHistoryItem) => {
  if (isBalanceType(item.type)) {
    return item.value >= 0 ? "bg-success/15 " : "bg-error/15 ";
  }
  if (isSubscriptionType(item.type)) return "bg-primary-100 ";
  return item.value >= 0 ? "bg-accent-teal/15 " : "bg-accent-amber/15 ";
};

// Icon text color
const getIconColor = (item: BalanceHistoryItem) => {
  if (isBalanceType(item.type)) {
    return item.value >= 0 ? "text-success " : "text-error ";
  }
  if (isSubscriptionType(item.type)) return "text-primary-700 ";
  return item.value >= 0 ? "text-primary-700 " : "text-warning ";
};
