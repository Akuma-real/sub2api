<template>
  ...
  >
    <div v-if="user" class="space-y-4">
      <div class="flex items-center gap-3 rounded-xl bg-surface-soft p-4">
        <div
      ...
      <div v-else-if="apiKeys.length === 0" class="py-8 text-center">
        <p class="text-sm text-muted">{{ t("admin.users.noApiKeys") }}</p>
      </div>
      <div
        v-else
        ref="scrollContainerRef"
        class="max-h-96 space-y-3 overflow-y-auto"
        @scroll="closeGroupSelector"
      >
        <div
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-muted">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="-mx-1 -my-0.5 flex cursor-pointer items-center gap-1 rounded-md px-1 py-0.5 transition-colors hover:bg-surface-card"
              ...
              >
                ...
                />
                <span v-else class="text-muted-soft italic">{{
                  t("admin.users.none")
                }}</span>
                <svg
            ...
            <div class="flex items-center gap-1">
              <span
                ...
                {{ formatDateTime(key.created_at) }}</span
              >
            </div>
          </div>
        </div>
  ...
  <Teleport to="body">
    <div
      ...
      ref="dropdownRef"
      class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-64 overflow-hidden rounded-xl bg-canvas shadow-card ring-1 ring-ink/5 duration-200"
      :style="{
    ...
    >
      <div class="max-h-64 overflow-y-auto p-1.5">
        ...
        >
          <span class="text-muted italic">{{ t("admin.users.none") }}</span>
          <svg
  ...
  if (groupSelectorKeyId.value === key.id) {
    ...
    if (buttonEl) {
      ...
      dropdownPosition.value = {
        top: openUpward
          ? rect.top - DROPDOWN_HEIGHT - DROPDOWN_GAP
...
const changeGroup = async (key: ApiKey, newGroupId: number | null) => {
  closeGroupSelector();
  if (key.group_id === newGroupId || (!key.group_id && newGroupId === null))
    return;
