    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-hairline bg-surface-soft p-3">
        <div class="flex items-center gap-2">
          ...
          <Icon name="arrowRight" size="sm" class="ml-auto text-muted-soft" />
        ...
        >
          ...
          >
            <div
              ...
              class="h-2 w-2 rounded-full bg-canvas"
            ></div>
          </div>
          <div class="flex-1">
      ...
      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-muted-soft">
...
const handleReplace = async () => {
  ...
  try {
    const result = await adminAPI.users.replaceGroup(
      props.user.id,
      props.oldGroup.id,
      selectedGroupId.value,
    );
    appStore.showSuccess(
      t("admin.users.replaceGroupSuccess", { count: result.migrated_keys }),
    );
    emit("success");
    emit("close");
  } catch (error) {
