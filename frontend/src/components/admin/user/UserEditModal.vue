        ...
        <div class="flex gap-2">
          <div class="relative flex-1">
            ...
            <button
              ...
              type="button"
              @click="copyPassword"
              class="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1 transition-colors hover:bg-surface-card"
              :class="passwordCopied ? 'text-success' : 'text-muted-soft'"
            >
              <svg
                v-if="passwordCopied"
                class="h-4 w-4"
...
const copyPassword = async () => {
  if (
    form.password &&
    (await copyToClipboard(form.password, t("admin.users.passwordCopied")))
  ) {
...
const handleUpdateUser = async () => {
  ...
  try {
    ...
    appStore.showError(
      e.response?.data?.detail || t("admin.users.failedToUpdate"),
    );
  } finally {
