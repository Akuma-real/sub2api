<template>
  ...
  >
    <div v-if="user" class="space-y-4">
      ...
      <div v-if="loading" class="py-10 text-center text-muted">{{ t('common.loading') }}</div>
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead>
            <tr class="border-b border-hairline-soft text-muted">
          ...
          <tbody>
            <tr v-for="row in quotas" :key="row.platform" class="border-b border-hairline-soft">
              <td class="px-3 py-2 font-mono text-ink">{{ row.platform }}</td>
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-soft transition-colors hover:bg-surface-card hover:text-warning disabled:opacity-50"
        ...
        <p class="mt-3 text-xs text-muted">{{ t('admin.users.platformQuota.hint') }}</p>
        <div class="mt-3">
