    ...
    >
      <div>
        <label class="input-label"
          >{{ t("admin.channelMonitor.form.name") }}
      ...
      <div>
        <label class="input-label"
          >{{ t("admin.channelMonitor.form.endpoint") }}
          <span class="text-error">*</span></label
        >
      ...
      <div>
        ...
        <p
          v-if="editing && editing.api_key_masked"
      ...
      <div>
        <label class="input-label">{{
          t("admin.channelMonitor.form.groupName")
        }}</label>
        <input
      ...
      <details class="rounded-lg border border-hairline bg-surface-soft/50 p-3">
...
const templateOptions = computed(() => {
  const items = templatesCache.value.filter(
    (t) => t.provider === form.provider,
  );
  return [
