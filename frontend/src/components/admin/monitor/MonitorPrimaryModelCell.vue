<template>
  <div class="flex items-center gap-2">
    <span class="text-sm text-ink">{{ row.primary_model }}</span>
    <HelpTooltip>
      <template #trigger>
        <span
      ...
      <div class="space-y-2">
        <div class="text-xs font-semibold text-canvas">
          {{ row.primary_model }}
          <span
        ...
        <div
          v-if="(row.extra_models?.length ?? 0) === 0"
          class="text-[11px] text-muted-soft"
        >
          {{ t("monitorCommon.extraModelsEmpty") }}
        </div>
        <div v-else class="space-y-1">
          <div
            class="text-[11px] font-semibold uppercase tracking-wide text-muted-soft"
          >
          ...
          <table class="w-full text-left text-[11px]">
            <thead>
              <tr class="text-muted-soft">
                <th class="py-0.5 pr-2 font-medium">
            ...
            <tbody>
              <tr v-for="m in row.extra_models_status || []" :key="m.model">
                <td class="py-0.5 pr-2 text-canvas">{{ m.model }}</td>
