<template>
  ...
  >
    ...
    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-muted-soft">
    ...
    <div v-else>
      ...
      <ul class="max-h-80 divide-y divide-hairline-soft overflow-y-auto rounded-lg border border-hairline">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-cream"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          <span v-if="m.provider === 'openai'" class="text-xs text-muted-soft">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
