<template>
  <BaseDialog
    :show="show"
    :title="
      t('admin.channelMonitor.template.applyPickerTitle', {
        name: templateName,
      })
    "
    @close="$emit('close')"
  >
    ...
    <div v-if="loading" class="py-6 text-center text-sm text-muted-soft">
      {{ t("common.loading") }}
    </div>
    ...
    <div v-else>
      ...
      <ul
        class="max-h-80 divide-y divide-hairline-soft overflow-y-auto rounded-lg border border-hairline"
      >
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-surface-soft"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          <span class="text-xs text-muted-soft">{{ m.provider }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-surface-card px-1.5 py-0.5 text-xs text-muted"
          >
            {{
      ...
      <div class="flex justify-end gap-2">
        ...
        >
          {{
            submitting
              ? t("common.submitting")
              : t("admin.channelMonitor.template.applyPickerConfirm", {
                  n: selectedIds.length,
                })
          }}
        </button>
...
async function fetchMonitors(id: number) {
  ...
  try {
    const { items } =
      await adminAPI.channelMonitorTemplate.listAssociatedMonitors(id);
...
async function handleApply() {
  ...
  try {
    ...
    appStore.showSuccess(
      t("admin.channelMonitor.template.applySuccess", { n: affected }),
    );
