  ...
  >
    ...
    >
      ...
      <div>
        ...
        <div
          class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-hairline bg-surface-soft px-4 py-3"
        >
          <div class="min-w-0">
            <div class="truncate text-sm text-body">
              {{ fileName || t("admin.proxies.dataImportSelectFile") }}
            </div>
            <div class="text-xs text-muted">JSON (.json)</div>
          </div>
          <button
            type="button"
...
const handleImport = async () => {
  ...
  try {
    ...
    if (res.proxy_failed > 0) {
      appStore.showError(
        t("admin.proxies.dataImportCompletedWithErrors", msgParams),
      );
    } else {
