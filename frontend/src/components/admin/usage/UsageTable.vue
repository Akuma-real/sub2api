<template>
    ...
    <div class="overflow-auto">
      ...
      >
        ...
        <template #cell-group="{ row }">
          <span
            v-if="row.group"
            class="inline-flex items-center rounded px-2 py-0.5 text-xs font-medium bg-primary-100 text-primary-800"
          >
            {{ row.group.name }}
          </span>
        ...
        <template #cell-tokens="{ row }">
          ...
          <div
            v-if="
              row.image_count > 0 && row.billing_mode === BILLING_MODE_IMAGE
            "
          ...
          >
            ...
            >
            <span class="text-muted-soft">({{ row.image_size || "2K" }})</span>
          </div>
          <!-- Token 请求 -->
          <div v-else class="flex items-center gap-1.5">
              ...
              >
                ...
                >
                  ...
                  <span
                    ...
                    class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-accent-amber/15 text-warning ring-1 ring-inset ring-accent-amber/30"
                    >1h</span
                  >
                  <span
                    v-if="row.cache_ttl_overridden"
                    :title="t('usage.cacheTtlOverriddenHint')"
                    class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-error/15 text-error ring-1 ring-inset ring-error/25 cursor-help"
                    >R</span
                  >
                </div>
              </div>
            ...
            >
              <div
                class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-surface-card transition-colors group-hover:bg-accent-teal/15"
              >
                <Icon
                  name="infoCircle"
                  size="xs"
                  class="text-muted-soft group-hover:text-accent-teal"
                />
              </div>
            </div>
          </div>
        ...
        <template #cell-first_token="{ row }">
          <span v-if="row.first_token_ms != null" class="text-sm text-body">{{
        ...
        <template #cell-user_agent="{ row }">
          <span
            v-if="row.user_agent"
            class="text-sm text-body block max-w-[320px] truncate"
        ...
        <template #cell-ip_address="{ row }">
          <span v-if="row.ip_address" class="text-sm font-mono text-body">{{
            row.ip_address
          }}</span>
          <span v-else class="text-sm text-muted-soft">-</span>
        </template>
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="whitespace-nowrap rounded-lg border border-surface-dark-elevated bg-surface-dark px-3 py-2.5 text-xs text-on-primary shadow-card"
      >
        <div class="space-y-1.5">
          <div>
            <div class="text-xs font-semibold text-muted-soft mb-1">
              ...
              >
                ...
                >
                  <span class="text-muted-soft flex items-center gap-1.5">
                    ...
                    <span
                      class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-accent-amber/20 text-warning ring-1 ring-inset ring-accent-amber/30"
                      >5m</span
                    >
                  </span>
                  <span class="font-medium text-on-primary">{{
            ...
            >
              <span class="text-muted-soft flex items-center gap-1.5">
                ...
                <span
                  class="inline-flex items-center rounded px-1 py-px text-[10px] font-medium leading-tight bg-error/15 text-error ring-1 ring-inset ring-error/25"
                  >R-{{
          ...
          <div
            class="flex items-center justify-between gap-6 border-t border-surface-dark-elevated pt-1.5"
          >
            <span class="text-muted-soft">{{ t("usage.totalTokens") }}</span>
            <span class="font-semibold text-accent-teal">{{
              (
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        <div class="space-y-1.5">
          <!-- Cost Breakdown -->
          <div class="mb-2 border-b border-surface-dark-elevated pb-1.5">
            <div class="text-xs font-semibold text-muted-soft mb-1">
              {{ t("usage.costDetails") }}
            </div>
            <div
            ...
            >
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-accent-teal"
                  >${{ imageUnitPrice(tooltipData).toFixed(6) }}</span
                >
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-muted-soft">{{
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
  const requestType = resolveUsageRequestType(row);
  if (requestType === "ws_v2") return "bg-primary-100 text-primary-800 ";
  if (requestType === "stream") return "bg-accent-teal/15 text-body-strong ";
  if (requestType === "sync") return "bg-surface-card text-body-strong ";
