<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        class="rounded-xl border border-accent-amber/30 bg-accent-amber/15 px-4 py-3 text-sm text-warning"
      >
      ...
      <div class="rounded-xl border border-hairline p-4">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-muted">
            {{ t("admin.usage.cleanup.loadingTasks") }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-muted">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-hairline-soft px-3 py-2 text-sm text-body"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-muted-soft">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
              ...
              <div class="flex flex-wrap items-center gap-4 text-xs text-muted">
                <span
                  >{{ t("admin.usage.cleanup.range") }}:
                  {{ formatRange(task) }}</span
                >
                <span
                  >{{ t("admin.usage.cleanup.deletedRows") }}:
                  {{ task.deleted_rows.toLocaleString() }}</span
                >
              </div>
              <div v-if="task.error_message" class="text-xs text-error">
                {{ task.error_message }}
              </div>
            </div>
          </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        >
          {{
            submitting
              ? t("admin.usage.cleanup.submitting")
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    pending: "bg-accent-amber/15 text-warning ",
    running: "bg-accent-teal/15 text-primary-700 ",
    succeeded: "bg-success/15 text-success ",
    failed: "bg-error/15 text-error ",
    canceled: "bg-hairline text-body ",
  };
  return map[status] || "bg-surface-card text-body";
};
...
const buildPayload = (): CreateUsageCleanupTaskRequest | null => {
  ...
  if (localFilters.value.request_type) {
    ...
    const legacyStream = requestTypeToLegacyStream(
      localFilters.value.request_type,
    );
