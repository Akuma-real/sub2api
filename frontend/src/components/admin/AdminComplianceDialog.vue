<template>
  ...
  >
    <div class="space-y-5">
      <div class="rounded-lg border border-warning/30 bg-warning/10 p-4 text-sm text-body">
        <div class="flex gap-3">
          <Icon name="exclamationTriangle" size="md" class="mt-0.5 flex-shrink-0 text-warning" />
          <div class="space-y-2">
            <p class="font-semibold text-ink">{{ t('adminCompliance.blockingNotice') }}</p>
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
        <section class="min-h-[320px] max-h-[46vh] overflow-y-auto rounded-lg border border-hairline bg-canvas p-5">
          <div class="legal-document-content" v-html="renderedDocument"></div>
        </section>

        <aside class="space-y-3 rounded-lg border border-hairline bg-surface-soft p-4 text-sm">
          <div>
            <p class="text-xs font-medium uppercase tracking-wide text-muted">
              {{ t('adminCompliance.version') }}
            </p>
            <p class="mt-1 break-all font-mono text-ink">
          ...
          <a
            ...
            class="inline-flex items-center gap-2 text-primary-600 underline underline-offset-4 hover:text-primary-700"
          >
      ...
      <div class="space-y-3">
        <label for="admin-compliance-phrase" class="block text-sm font-semibold text-ink">
          {{ t('adminCompliance.inputLabel') }}
        </label>
        <div class="rounded-lg bg-surface-card px-3 py-2 font-mono text-sm text-ink">
          {{ expectedPhrase }}
        </div>
        <Input
...
.legal-document-content :deep(p) {
  @apply mb-4 text-sm text-body;
}

.legal-document-content :deep(ul),
.legal-document-content :deep(ol) {
  @apply mb-4 pl-6 text-sm text-body;
}
