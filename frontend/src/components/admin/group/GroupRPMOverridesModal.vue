    ...
    <div v-if="group" class="space-y-4">
      ...
      <div class="rounded-lg border border-hairline p-3">
        ...
        <div
          ...
          class="mt-3 flex items-center justify-end border-t border-hairline-soft pt-3"
        >
          <button
            type="button"
            :disabled="clearing"
            class="rounded-lg border border-error/25 bg-error/15 px-3 py-1.5 text-sm font-medium text-error transition-colors hover:bg-error/15 disabled:opacity-50"
            @click="clearAllLocal"
          >
            <Icon
              v-if="clearing"
...
const isDirty = computed(() => {
  ...
  const serverMap = new Map(
    serverEntries.value.map((e) => [e.user_id, e.rpm_override]),
  );
  return localEntries.value.some(
    (e) => serverMap.get(e.user_id) !== e.rpm_override,
  );
});
...
const loadEntries = async () => {
  ...
  loading.value = true;
  try {
    serverEntries.value = await adminAPI.groups.getGroupRPMOverrides(
      props.group.id,
    );
