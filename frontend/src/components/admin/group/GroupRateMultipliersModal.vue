<template>
  ...
  >
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div
        class="flex flex-wrap items-center gap-3 rounded-lg bg-surface-soft px-4 py-2.5 text-sm"
      >
        <span
      ...
      <div class="rounded-lg border border-hairline p-3">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-body">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-lg border border-hairline bg-canvas shadow-card"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-surface-soft"
                @click="selectUser(user)"
              >
                <span class="text-muted-soft">#{{ user.id }}</span>
                <span class="text-ink">{{ user.username || user.email }}</span>
                <span v-if="user.username" class="text-xs text-muted-soft">{{
                  user.email
                }}</span>
              </button>
            </div>
        ...
        <div
          ...
          class="mt-3 flex items-center gap-3 border-t border-hairline-soft pt-3"
        >
          ...
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-hairline bg-canvas px-2 py-1 text-center text-sm transition-colors focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/20"
              placeholder="0.5"
            />
            <button
              type="button"
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-lg border border-error/25 bg-error/15 px-3 py-1.5 text-sm font-medium text-error transition-colors hover:bg-error/15"
              @click="clearAllLocal"
            >
      ...
      <div v-else>
        ...
        <div
          v-if="localEntries.length === 0"
          class="py-6 text-center text-sm text-muted-soft"
        >
              ...
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-hairline bg-surface-soft">
                    <th
                      class="px-3 py-2 text-left text-xs font-medium text-muted"
                    >
                    ...
                    <th
                      v-if="showFinalRate"
                      class="px-3 py-2 text-left text-xs font-medium text-primary-600"
                    >
                ...
                <tbody class="divide-y divide-hairline-soft">
                  ...
                  >
                    <td class="px-3 py-2 text-body">{{ entry.user_email }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-muted-soft">
                      {{ entry.user_id }}
                    </td>
                    <td class="whitespace-nowrap px-3 py-2 text-ink">
                      {{ entry.user_name || "-" }}
                    </td>
                    <td
                      class="max-w-[160px] truncate px-3 py-2 text-muted"
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded border border-hairline bg-canvas px-2 py-1 text-center text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/20"
                        @change="
                          updateLocalRate(
                            entry.user_id,
                    ...
                    <td
                      v-if="showFinalRate"
                      class="whitespace-nowrap px-3 py-2 font-medium text-primary-600"
                    >
                    ...
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-muted-soft transition-colors hover:bg-error/15 hover:text-error"
...
const isDirty = computed(() => {
  ...
  const serverMap = new Map(
    serverEntries.value.map((e) => [e.user_id, e.rate_multiplier ?? null]),
  );
  return localEntries.value.some(
    (e) => serverMap.get(e.user_id) !== (e.rate_multiplier ?? null),
  );
});
...
const adjustPage = () => {
  const totalPages = Math.max(
    1,
    Math.ceil(localEntries.value.length / pageSize.value),
  );
...
const applyBatchFactor = () => {
  ...
  for (const entry of localEntries.value) {
    if (entry.rate_multiplier != null) {
      entry.rate_multiplier = parseFloat(
        (entry.rate_multiplier * batchFactor.value).toFixed(6),
      );
    }
  }
  batchFactor.value = null;
