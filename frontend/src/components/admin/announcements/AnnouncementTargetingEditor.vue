<template>
  <div class="rounded-lg border border-hairline bg-surface-soft p-4">
    ...
    <div v-if="mode === 'custom'" class="mt-4 space-y-4">
      ...
      <div
        v-if="anyOf.length === 0"
        class="rounded-xl border border-dashed border-hairline p-4 text-sm text-muted"
      >
        {{ t("admin.announcements.form.targetingCustom") }}:
        {{ t("admin.announcements.form.addOrGroup") }}
      </div>
      ...
      <div
        ...
        :key="groupIndex"
        class="rounded-lg border border-hairline bg-canvas p-4 shadow-sm"
      >
          ...
          <div class="min-w-0">
            <div class="text-sm font-medium text-ink">
              ...
              <span class="ml-2 text-xs font-normal text-muted"
                >AND ({{ group.all_of?.length || 0 }}/50)</span
              >
            </div>
            <div class="mt-1 text-xs text-muted">
        ...
        <div class="mt-4 space-y-3">
          <div
            ...
            :key="condIndex"
            class="rounded-xl border border-hairline bg-surface-soft p-3"
          >
      ...
      <div
        ...
        class="rounded-xl border border-error/25 bg-error/15 p-3 text-sm text-error"
      >
        {{ validationError }}
      </div>
    </div>
...
const mode = computed<Mode>(() =>
  anyOf.value.length === 0 ? "all" : "custom",
);

const conditionTypeOptions = computed(() => [
  {
    value: "subscription",
    label: t("admin.announcements.form.conditionSubscription"),
  },
...
function setMode(next: Mode) {
  ...
  if (anyOf.value.length === 0) {
    emit("update:modelValue", {
      any_of: [{ all_of: [defaultSubscriptionCondition()] }],
    });
  }
}
...
function updateTargeting(mutator: (draft: TargetingDraft) => void) {
  const draft: TargetingDraft = JSON.parse(
    JSON.stringify(props.modelValue ?? { any_of: [] }),
  );
...
function setConditionType(
  groupIndex: number,
  condIndex: number,
  nextType: AnnouncementConditionType,
) {
  updateTargeting((draft) => {
    const group = draft.any_of[groupIndex];
    if (!group?.all_of) return;
...
const subscriptionSelections = reactive<
  Record<number, Record<number, number[]>>
>({});

function ensureSelectionPath(groupIndex: number, condIndex: number) {
  if (!subscriptionSelections[groupIndex])
    subscriptionSelections[groupIndex] = {};
  if (!subscriptionSelections[groupIndex][condIndex])
    ...
    for (let gi = 0; gi < groups.length; gi++) {
      ...
      for (let ci = 0; ci < allOf.length; ci++) {
        ...
        if (c?.type === "subscription") {
          ...
          if (
            JSON.stringify(newIds.sort()) !== JSON.stringify(currentIds.sort())
          ) {
...
watch(
  ...
  () => {
    ...
    syncTimeout = setTimeout(() => {
      ...
      const newTargeting: TargetingDraft = JSON.parse(
        JSON.stringify(props.modelValue ?? { any_of: [] }),
      );
...
const validationError = computed(() => {
  ...
  for (const g of groups) {
    ...
    if (allOf.length === 0)
      return t("admin.announcements.form.addAndCondition");
