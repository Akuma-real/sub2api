<template>
  <div
    ...
    :class="
      isEmpty ? 'border-error bg-error/15 ' : 'border-hairline bg-canvas '
    "
  >
    <!-- Token mode: context range + prices ($/MTok) -->
    <template v-if="mode === 'token'">
      <div class="w-20">
        <label class="text-xs text-muted-soft">Min</label>
        <input
          :value="interval.min_tokens"
          @input="
            emitField(
              'min_tokens',
              toInt(($event.target as HTMLInputElement).value),
            )
          "
          type="number"
          min="0"
      ...
      <div class="w-20">
        <label class="text-xs text-muted-soft"
          >Max <span class="text-muted-soft">(含)</span></label
        >
        <input
          ...
          @input="
            emitField(
              'max_tokens',
              toIntOrNull(($event.target as HTMLInputElement).value),
            )
          "
          type="number"
          min="0"
      ...
      <div class="flex-1">
        <label class="text-xs text-muted-soft"
          ...
          <span v-if="isEmpty" class="text-error">*</span>
          <span class="text-muted-soft">$/M</span></label
        >
        <input
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-muted-soft"
          ...
          <span v-if="isEmpty" class="text-error">*</span>
          <span class="text-muted-soft">$</span></label
        >
        <input
    ...
    <button
      type="button"
      @click="emit('remove')"
      class="mt-4 rounded p-0.5 text-muted-soft hover:text-error"
    >
      <Icon name="x" size="sm" />
    </button>
  </div>
