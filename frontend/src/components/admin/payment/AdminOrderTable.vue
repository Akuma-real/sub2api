  ...
  <div class="space-y-4">
    ...
    <DataTable :columns="columns" :data="orders" :loading="loading">
      ...
      <template #cell-pay_amount="{ value, row }">
        <div class="text-sm">
          <span class="font-medium text-ink">¥{{ value.toFixed(2) }}</span>
          <span
            v-if="row.fee_rate > 0"
            class="ml-1 text-xs text-muted-soft"
            :title="t('payment.orders.fee') + ': ' + row.fee_rate + '%'"
          >
            ({{ row.fee_rate }}%)
          </span>
          <div v-if="row.amount !== row.pay_amount" class="text-xs text-muted">
      ...
      <template #cell-actions="{ row }">
        <div class="flex items-center gap-2">
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-surface-soft hover:text-body"
          >
          ...
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-accent-amber/15 hover:text-warning"
          >
            <Icon name="x" size="sm" />
