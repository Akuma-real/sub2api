...
const anthropicPresetMappings = [
  {
    ...
    color: "bg-accent-teal/15 text-primary-700 hover:bg-accent-teal/25 ",
  },
  {
    label: "Sonnet 4.5",
    from: "claude-sonnet-4-5-20250929",
    to: "claude-sonnet-4-5-20250929",
    color: "bg-primary-100 text-primary-700 hover:bg-primary-100 ",
  },
  {
    label: "Sonnet 4.6",
  ...
  },
  {
    ...
    color: "bg-success/15 text-success hover:bg-success/15 ",
  },
  {
    label: "Haiku 4.5",
  ...
  },
  {
    ...
    from: "claude-opus-4-6",
    to: "claude-sonnet-4-5-20250929",
    color: "bg-accent-amber/15 text-warning hover:bg-accent-amber/15 ",
  },
];
...
const openaiPresetMappings = [
  ...
  },
  {
    ...
    color: "bg-accent-teal/15 text-accent-teal hover:bg-accent-teal/15 ",
  },
  {
    label: "GPT-5.2",
    from: "gpt-5.2",
    to: "gpt-5.2",
    color: "bg-error/15 text-error hover:bg-error/15 ",
  },
  {
    label: "GPT-5.5",
    from: "gpt-5.5",
  ...
  },
  {
    ...
    from: "gpt-5.4",
    to: "gpt-5.4",
    color: "bg-error/15 text-error hover:bg-error/20 ",
  },
  {
    label: "Haiku→5.4",
    from: "claude-haiku-4-5-20251001",
...
const bedrockPresetMappings = [
  ...
  },
  {
    ...
    to: "us.anthropic.claude-opus-4-5-20251101-v1:0",
    color: "bg-primary-100 text-primary-700 hover:bg-primary-100 ",
  },
  {
    label: "Sonnet 4.5",
...
let _antigravityDefaultMappingsCache: { from: string; to: string }[] | null =
  null;
...
> {
  ...
  try {
    ...
    _antigravityDefaultMappingsCache = Object.entries(mapping).map(
      ([from, to]) => ({ from, to }),
    );
  } catch (e) {
    console.warn(
  ...
  if (mode === "whitelist") {
    ...
    for (const m of modelMappings) {
      ...
      if (!isValidWildcardPattern(from)) {
        console.warn(
          `[buildModelMappingObject] 无效的通配符格式，跳过: ${from}`,
        );
        continue;
      }
      // to 不允许包含通配符
      if (to.includes("*")) {
        console.warn(
          `[buildModelMappingObject] 目标模型不能包含通配符，跳过: ${from} -> ${to}`,
        );
        continue;
      }
      mapping[from] = to;
