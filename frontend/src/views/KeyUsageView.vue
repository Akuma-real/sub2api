  ...
  <div class="relative flex min-h-screen flex-col bg-canvas text-ink">
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1 class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-ink">
          {{ t("keyUsage.title") }}
        </h1>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div
              class="absolute left-4 top-1/2 -translate-y-1/2 text-muted-soft"
            >
              <svg
                class="w-5 h-5"
                viewBox="0 0 24 24"
            ...
            <input
              ...
              class="input-ring w-full h-12 pl-12 pr-12 rounded-xl border border-hairline bg-canvas text-sm text-ink placeholder:text-muted-soft transition-all"
              @keydown.enter="queryKey"
            />
            <button
              @click="keyVisible = !keyVisible"
              class="absolute right-4 top-1/2 -translate-y-1/2 text-muted-soft hover:text-body transition-colors"
            >
              <svg
                v-if="!keyVisible"
                class="w-5 h-5"
        ...
        <p class="text-xs text-muted-soft mt-3 text-center">
          ...
          <div class="flex flex-wrap items-center gap-2 justify-center">
            ...
            <button
              ...
              :class="
                currentRange === range.key
                  ...
                  : 'border-hairline bg-canvas text-body hover:border-primary-300 '
              "
            >
              {{ range.label }}
            </button>
            ...
            >
              <input
                ...
                type="date"
                class="input-ring text-xs px-2 py-1.5 rounded-lg border border-hairline bg-canvas text-ink"
              />
      ...
      <div v-if="showResults">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-lg border border-hairline bg-canvas p-8">
              ...
              <div class="flex justify-center">
                <div class="skeleton w-44 h-44 rounded-full"></div>
              </div>
            </div>
            <div class="rounded-lg border border-hairline bg-canvas p-8">
              <div class="skeleton h-5 w-24 mb-6"></div>
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div
            v-if="statusInfo"
            class="fade-up flex items-center justify-center mb-2"
          >
            <div
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-hairline bg-canvas/90 shadow-sm "
            >
              <span
                class="w-2.5 h-2.5 rounded-full pulse-dot"
                :class="statusInfo.isActive ? 'bg-success' : 'bg-error'"
              ></span>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up rounded-lg border border-hairline bg-canvas/90 p-8  transition-all duration-300 hover:shadow-card"
                  ...
                  <svg class="w-44 h-44" viewBox="0 0 160 160">
                    ...
                    <defs>
                      ...
                      >
                        <stop
                          offset="0%"
                          :stop-color="RING_GRADIENTS[i % 4].from"
                        />
                        <stop
                          offset="100%"
                          :stop-color="RING_GRADIENTS[i % 4].to"
                        />
                      </linearGradient>
                    </defs>
                  </svg>
                  ...
                  >
                    ...
                    <template v-else>
                      ...
                      <p
                        ...
                        class="text-xs text-muted-soft mt-0.5 tabular-nums"
                      >
          ...
          <div
            ...
            class="fade-up fade-up-delay-3 rounded-lg border border-hairline bg-canvas/90  overflow-hidden"
          >
            <div class="px-8 py-5 border-b border-hairline">
              <h3
                class="text-sm font-semibold uppercase tracking-wider text-muted"
              >
          ...
          >
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-px bg-surface-card">
              <div
                v-for="(cell, i) in usageStatCells"
                :key="i"
                class="bg-canvas px-6 py-4"
              >
          ...
          <div
            v-if="showDailyUsage"
            class="fade-up fade-up-delay-4 overflow-hidden rounded-lg border border-hairline bg-canvas/90"
          >
            <div class="flex flex-col gap-3 border-b border-hairline px-8 py-5 sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-muted">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-lg border border-hairline bg-canvas p-0.5">
                <button
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-ink">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-body">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-body">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-body">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-body">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-body">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-ink">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div
            ...
            class="fade-up fade-up-delay-4 rounded-lg border border-hairline bg-canvas/90  overflow-hidden"
          >
            <div class="px-8 py-5 border-b border-hairline">
              <h3
                class="text-sm font-semibold uppercase tracking-wider text-muted"
            ...
            <div class="overflow-x-auto">
              <table class="w-full">
                ...
                <tbody>
                  ...
                  >
                    <td
                      class="px-4 py-3 text-sm font-medium whitespace-nowrap text-ink"
                    >
                      {{ m.model || "-" }}
                    </td>
                    <td
                      class="px-4 py-3 text-sm tabular-nums text-right text-body"
                    >
                      {{ fmtNum(m.requests) }}
                    </td>
                    <td
                    ...
                    <td
                      class="px-4 py-3 text-sm tabular-nums text-right font-medium text-ink"
                    >
    ...
    <footer class="relative z-10 border-t border-hairline/50 px-6 py-8">
      <div
...
const RING_GRADIENTS = [
  { from: "#cc785c", to: "#e8a55a" },
  { from: "#181715", to: "#6c6a64" },
  { from: "#5db872", to: "#5db872" },
  { from: "#e8a55a", to: "#e8a55a" },
];
  ...
  if (data.mode === "quota_limited") {
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        const pct =
          rl.limit > 0
            ? Math.min(Math.round((rl.used / rl.limit) * 100), 100)
            : 0;
        items.push({
...
const usageStatCells = computed<StatCell[]>(() => {
  ...
  return [
    ...
    {
      label: t("keyUsage.todayInputTokens"),
      value: fmtNum(today.input_tokens),
    },
    {
      label: t("keyUsage.todayOutputTokens"),
      value: fmtNum(today.output_tokens),
    },
    { label: t("keyUsage.todayTokens"), value: fmtNum(today.total_tokens) },
    {
      label: t("keyUsage.todayCacheCreation"),
      value: fmtNum(today.cache_creation_tokens),
    },
    {
      label: t("keyUsage.todayCacheRead"),
      value: fmtNum(today.cache_read_tokens),
    },
    { label: t("keyUsage.todayCost"), value: usd(today.actual_cost) },
    {
      label: t("keyUsage.rpmTpm"),
      value: `${usage.rpm || 0} / ${usage.tpm || 0}`,
    },
    { label: t("keyUsage.totalRequests"), value: fmtNum(total.requests) },
    {
      label: t("keyUsage.totalInputTokens"),
      value: fmtNum(total.input_tokens),
    },
    {
      label: t("keyUsage.totalOutputTokens"),
      value: fmtNum(total.output_tokens),
    },
    {
      label: t("keyUsage.totalTokensLabel"),
      value: fmtNum(total.total_tokens),
    },
    {
      label: t("keyUsage.totalCacheCreation"),
      value: fmtNum(total.cache_creation_tokens),
    },
    {
      label: t("keyUsage.totalCacheRead"),
      value: fmtNum(total.cache_read_tokens),
    },
    { label: t("keyUsage.totalCost"), value: usd(total.actual_cost) },
    {
      label: t("keyUsage.avgDuration"),
      value: usage.average_duration_ms
...
async function fetchUsage(key: string) {
  ...
  if (!res.ok) {
    ...
    const msg =
      body?.error?.message ||
      body?.message ||
      `${t("keyUsage.queryFailed")} (${res.status})`;
    throw new Error(msg);
  }
...
async function queryKey() {
  ...
  try {
    ...
    showLoading.value = false;
    appStore.showError(
      (err as Error).message || t("keyUsage.queryFailedRetry"),
    );
  } finally {
