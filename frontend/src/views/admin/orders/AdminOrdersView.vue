        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            <button
              ...
              class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-body hover:bg-surface-card"
            >
            ...
            <button
              ...
              class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-warning hover:bg-accent-amber/15"
            >
              <Icon name="x" size="sm" />
            ...
            <button
              ...
              class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-primary-700 hover:bg-accent-teal/10"
            >
            ...
            <template v-if="row.order_type !== 'vip' && row.status === 'REFUND_REQUESTED'">
              <span
                ...
                >{{ row.order_type === "balance" ? "$" : "¥"
                }}{{ row.refund_amount.toFixed(2) }}</span
              >
              <button
    ...
    >
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <p class="text-xs text-muted">{{ t("payment.orders.orderNo") }}</p>
          ...
          <div>
            <p class="text-xs text-muted">{{ t("payment.orders.amount") }}</p>
            <p class="text-sm font-medium text-ink">
          ...
          <div>
            <p class="text-xs text-muted">{{ t("payment.admin.feeRate") }}</p>
            <p class="text-sm text-body">{{ selectedOrder.fee_rate }}%</p>
          </div>
          <div>
            <p class="text-xs text-muted">
          ...
          <div v-if="selectedOrder.refund_reason" class="col-span-2">
            <p class="text-xs text-muted">
              {{ t("payment.admin.refundReason") }}
            </p>
            <p class="text-sm text-body">{{ selectedOrder.refund_reason }}</p>
          </div>
          <!-- Refund request info -->
          <div
            v-if="selectedOrder.refund_requested_at"
            class="col-span-2 border-t border-hairline pt-3"
          >
            <p class="mb-2 text-xs font-medium text-primary-700">
        ...
        >
          ...
          <div class="max-h-48 space-y-2 overflow-y-auto">
            <div
              v-for="log in orderAuditLogs"
              :key="log.id"
              class="rounded-lg border border-hairline-soft bg-surface-soft p-2.5"
            >
              ...
              <div v-if="log.detail" class="mt-1 break-all text-xs text-muted">
                {{ log.detail }}
              </div>
              <div v-if="log.operator" class="mt-1 text-xs text-muted-soft">
