  ...
  <AppLayout>
        ...
        >
          ...
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-ink">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-muted">
                {{ row.out_trade_no }}
              </div>
            </div>
          </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-hairline-soft bg-surface-soft p-4">
  ...
  setup(cellProps, { emit }) {
    return () =>
      h("div", { class: "space-y-0.5" }, [
        h("div", { class: "font-mono text-sm text-ink " }, `#${cellProps.id}`),
        h(
          cellProps.clickable ? "button" : "div",
          {
            class: cellProps.clickable
              ...
              : "max-w-56 truncate text-sm text-body ",
        ...
        h(
          "div",
          { class: "max-w-56 truncate text-sm text-muted " },
          cellProps.username || "-",
        ),
      ]);
  },
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h("span", { class: "text-sm text-muted-soft " }, "-");
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () =>
      h(
        "div",
        { class: "rounded-lg border border-hairline-soft bg-canvas p-3 " },
        [
          ...
          h(
            "div",
            {
              class: statProps.mono
                ? "mt-1 font-mono text-base font-semibold text-ink "
