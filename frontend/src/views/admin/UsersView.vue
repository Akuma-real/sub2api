  ...
  <AppLayout>
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 w-48 rounded-lg border border-hairline bg-canvas py-1 shadow-lg"
                >
                  <!-- Built-in filters -->
                  <button
                    ...
                    class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-body hover:bg-surface-card"
                  >
                  ...
                  <div
                    ...
                    class="my-1 border-t border-hairline-soft"
                  ></div>
              ...
              <div class="relative" ref="columnDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-hairline bg-canvas py-1 shadow-lg"
                >
                  <button
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      :class="isForcedVisibleColumn(col.key) ? 'text-muted-soft' : 'text-primary-500'"
                      :stroke-width="2"
                    />
                  </button>
                </div>
        ...
        >
          ...
          <template #cell-notes="{ value }">
            <div class="max-w-xs">
              <span
                ...
                class="block truncate text-sm text-body"
              >
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                <Icon name="shield" size="xs" class="h-3.5 w-3.5 text-accent-amber" />
                <span class="font-medium text-primary-600">{{ getUserGroups(row).exclusive.length }}</span>
                <span class="text-muted">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-surface-dark px-2.5 py-1.5 text-xs text-on-dark opacity-0 shadow-lg transition-opacity duration-75 group-hover/ex:opacity-100"
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-surface-dark"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-hairline bg-canvas py-1 text-xs shadow-xl"
                >
                  <div class="border-b border-hairline-soft px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-soft">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-body transition-colors hover:bg-primary-50 hover:text-primary-600"
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-muted-soft" />
                <span class="font-medium text-body">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-muted-soft">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-surface-dark px-2.5 py-1.5 text-xs text-on-dark opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-surface-dark"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
            ...
            <span v-else class="text-xs text-muted-soft">-</span>
          </template>
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-cream px-2 py-1 text-xs text-muted-soft"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-ink underline decoration-dashed decoration-gray-300 underline-offset-4 transition-colors hover:text-primary-600"
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-surface-dark px-2 py-1 text-xs text-on-dark opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100">
                  {{ t('admin.users.balanceHistoryTip') }}
                  <div class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-surface-dark"></div>
                </div>
              </div>
              <button
                @click.stop="handleDeposit(row)"
                class="rounded px-2 py-0.5 text-xs font-medium text-success transition-colors hover:bg-success/10"
          ...
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  type="button"
                  class="flex items-center gap-1 rounded px-1 py-0.5 transition-colors hover:bg-hairline-soft"
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 min-w-[120px] rounded-lg border border-hairline bg-canvas py-1 shadow-lg"
                >
                  <button
                    v-for="metric in (['today', 'total'] as const)"
                    :key="metric"
                    type="button"
                    class="flex w-full items-center justify-between gap-3 px-3 py-1.5 text-left text-xs normal-case tracking-normal hover:bg-surface-card"
                  ...
                  <div class="mt-1 border-t border-hairline-soft px-3 py-1 text-[10px] normal-case tracking-normal text-muted-soft">
            ...
            <div class="flex items-center gap-1.5">
              <span
                :class="[
                  ...
                  value === 'active' ? 'bg-success/100' : 'bg-error/150'
                ]"
              ></span>
            ...
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'hover:bg-surface-soft hover:text-primary-600'
              ...
              <button
                ...
                class="action-menu-trigger flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-surface-card hover:text-ink"
                :class="{ 'bg-surface-card text-ink': activeMenuId === row.id }"
              >
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-xl bg-canvas shadow-lg ring-1 ring-ink/5"
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-body hover:bg-surface-card"
              >
                <Icon name="key" size="sm" class="text-muted-soft" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-body hover:bg-surface-card"
              >
                <Icon name="users" size="sm" class="text-muted-soft" :stroke-width="2" />
                {{ t('admin.users.groups') }}
              </button>

              <div class="my-1 border-t border-hairline-soft"></div>
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-body hover:bg-surface-card"
              >
                <Icon name="plus" size="sm" class="text-accent-teal" :stroke-width="2" />
                {{ t('admin.users.deposit') }}
              </button>
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-body hover:bg-surface-card"
              >
                <svg class="h-4 w-4 text-accent-amber" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-body hover:bg-surface-card"
              >
                <Icon name="chartBar" size="sm" class="text-muted-soft" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-body hover:bg-surface-card"
              >
                <Icon name="dollar" size="sm" class="text-muted-soft" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-error hover:bg-error/15"
              >
