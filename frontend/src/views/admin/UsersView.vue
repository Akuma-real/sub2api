  ...
  <AppLayout>
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 w-48 rounded-lg border border-hairline bg-canvas py-1 shadow-card"
                >
                  <!-- Built-in filters -->
                  <button
              ...
              <div class="relative" ref="columnDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-hairline bg-canvas py-1 shadow-card"
                >
                  <button
      ...
      <template #table>
        ...
        >
            ...
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                ...
                <span class="font-medium text-primary-700">{{
                  getUserGroups(row).exclusive.length
                }}</span>
                ...
                <div
                  ...
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-surface-dark px-2.5 py-1.5 text-xs text-on-primary opacity-0 shadow-card transition-opacity duration-75 group-hover/ex:opacity-100"
                >
                  <div
                    class="absolute left-4 bottom-full border-4 border-transparent border-b-surface-dark"
                  ></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-hairline bg-canvas py-1 text-xs shadow-card"
                >
                  <div
                    class="border-b border-hairline-soft px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-soft"
                  >
              ...
              >
                <Icon
                  name="globe"
                  size="xs"
                  class="h-3.5 w-3.5 text-muted-soft"
                />
                ...
                <!-- Tooltip: 向下弹出 -->
                <div
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-surface-dark px-2.5 py-1.5 text-xs text-on-primary opacity-0 shadow-card transition-opacity duration-75 group-hover/pub:opacity-100"
                >
                  <div
                    class="absolute left-4 bottom-full border-4 border-transparent border-b-surface-dark"
                  ></div>
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-surface-soft px-2 py-1 text-xs text-muted-soft"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-ink underline decoration-dashed decoration-muted-soft underline-offset-4 transition-colors hover:text-primary-600"
                ...
                <!-- Instant tooltip -->
                <div
                  class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-surface-dark px-2 py-1 text-xs text-on-primary opacity-0 shadow-card transition-opacity duration-75 group-hover:opacity-100"
                >
                  {{ t("admin.users.balanceHistoryTip") }}
                  <div
                    class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-surface-dark"
                  ></div>
                </div>
              </div>
              <button
                @click.stop="handleDeposit(row)"
                class="rounded px-2 py-0.5 text-xs font-medium text-success transition-colors hover:bg-success/15"
            ...
            <div class="flex items-center gap-1.5">
              <span
                :class="[
                  ...
                  value === 'active' ? 'bg-success' : 'bg-error',
                ]"
              ></span>
            ...
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'hover:bg-accent-amber/15 hover:text-warning '
              ...
              <button
                ...
                :class="{
                  'bg-surface-card text-ink ': activeMenuId === row.id,
                }"
              >
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-xl bg-canvas shadow-card ring-1 ring-ink/5"
        :style="{
  ...
  try {
    ...
    if (saved) {
      ...
      parsed
        .filter(
          (key) =>
            !REMOVED_COLUMNS.has(key) && !FORCED_VISIBLE_COLUMNS.has(key),
        )
