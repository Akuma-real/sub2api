          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center space-x-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-surface-card hover:text-body"
                :title="t('common.edit')"
              >
      ...
      >
        <div>
          <label class="input-label">
            {{ t("admin.promo.code") }}
            <span class="ml-1 text-xs font-normal text-muted-soft"
              >({{ t("admin.promo.autoGenerate") }})</span
            >
          </label>
          <input
        ...
        <div>
          <label class="input-label">
            ...
            <span class="ml-1 text-xs font-normal text-muted-soft"
              >({{ t("admin.promo.zeroUnlimited") }})</span
            >
          </label>
          <input
    ...
    >
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :key="usage.id"
          class="flex items-center justify-between rounded-lg border border-hairline p-3"
        >
          <div class="flex items-center gap-3">
            <div
              class="flex h-8 w-8 items-center justify-center rounded-full bg-success/15"
            >
            ...
            <div>
              <p class="text-sm font-medium text-ink">
                {{
                  usage.user?.email ||
                  t("admin.promo.userPrefix", { id: usage.user_id })
                }}
              </p>
...
const columns = computed<Column[]>(() => [
  ...
  {
    key: "bonus_amount",
    label: t("admin.promo.columns.bonusAmount"),
    sortable: true,
  },
  ...
  {
    key: "expires_at",
    label: t("admin.promo.columns.expiresAt"),
    sortable: true,
  },
  {
    key: "created_at",
    label: t("admin.promo.columns.createdAt"),
    sortable: true,
  },
...
const getStatusLabel = (status: string, row: PromoCode) => {
  ...
  return status === "active"
    ? t("admin.promo.statusActive")
    : t("admin.promo.statusDisabled");
};
...
const handleCreate = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.promo.failedToCreate"),
    );
  } finally {
...
const handleUpdate = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.promo.failedToUpdate"),
    );
  } finally {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.promo.failedToDelete"),
    );
  }
};
...
const loadUsages = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.promo.failedToLoadUsages"),
    );
  } finally {
