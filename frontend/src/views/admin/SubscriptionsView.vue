<template>
  <AppLayout>
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <div class="relative w-full sm:w-64" data-filter-user-search>
              ...
              <div
                ...
                "
                class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-hairline bg-canvas shadow-card"
              >
                <div
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            ...
            <div class="relative" ref="columnDropdownRef">
              ...
              <div
                ...
                class="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-lg border border-hairline bg-canvas shadow-card"
              >
                <div class="p-2">
                  <!-- User column mode selection -->
                  <div class="mb-2 border-b border-hairline pb-2">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="font-medium text-ink">
                {{
                  userColumnMode === "email"
                    ? row.user?.email ||
                      t("admin.redeem.userPrefix", { id: row.user_id })
                    : row.user?.username || "-"
                }}
              </span>
            </div>
            ...
            <div class="min-w-[280px] space-y-2">
              ...
              <div v-if="row.group?.daily_limit_usd" class="usage-row">
                <div class="flex items-center gap-2">
                  ...
                  <div class="h-1.5 flex-1 rounded-full bg-hairline">
                    <div
              ...
              <div
                v-if="
                  ...
                  !row.group?.monthly_limit_usd
                "
                class="flex items-center gap-2 rounded-lg bg-surface-soft px-3 py-2"
              >
          ...
          <template #cell-expires_at="{ value }">
            <div v-if="value">
              <span
                class="text-sm"
                :class="
                  isExpiringSoon(value) ? 'text-warning ' : 'text-body '
                "
              >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-accent-teal/10 hover:text-primary-700"
              >
              ...
              <button
                ...
                "
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-accent-amber/15 hover:text-warning disabled:cursor-not-allowed disabled:opacity-50"
              >
      ...
      <template #footer>
        <div v-if="extendingSubscription" class="flex justify-end gap-3">
          ...
          >
            {{
              submitting
                ? t("admin.subscriptions.adjusting")
                : t("admin.subscriptions.adjust")
            }}
          </button>
        </div>
    ...
    <ConfirmDialog
      ...
      :message="
        t('admin.subscriptions.revokeConfirm', {
    ...
    <teleport to="body">
      <transition name="modal">
        <div
          ...
          @mousedown.self="showGuideModal = false"
        >
          <div
          ...
          ></div>
          <div
            class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-canvas p-6 shadow-card"
          >
            <button
              type="button"
              class="absolute right-4 top-4 text-muted-soft hover:text-body"
              @click="showGuideModal = false"
            >
              <svg
                class="h-5 w-5"
            ...
            <div class="mb-5">
              ...
              <ol class="ml-8 list-decimal space-y-1 text-sm text-body">
            ...
            <div class="mb-5">
              ...
              <div
                class="ml-8 overflow-hidden rounded-lg border border-hairline"
              >
                <table class="w-full text-sm">
                  <tbody>
                    <tr
                      v-for="(row, i) in guideActionRows"
                      :key="i"
                      class="border-b border-hairline-soft last:border-0"
                    >
                      <td
                        class="whitespace-nowrap bg-surface-soft px-3 py-2 font-medium text-body"
                      >
                        {{ row.action }}
                      </td>
                      <td class="px-3 py-2 text-body">{{ row.desc }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <!-- Tip -->
            <div
              class="rounded-lg bg-accent-teal/10 p-3 text-xs text-primary-700"
            >
...
const guideActionRows = computed(() => [
  {
    action: t("admin.subscriptions.guide.actions.adjust"),
    desc: t("admin.subscriptions.guide.actions.adjustDesc"),
  },
  {
    action: t("admin.subscriptions.guide.actions.resetQuota"),
    desc: t("admin.subscriptions.guide.actions.resetQuotaDesc"),
  },
  {
    action: t("admin.subscriptions.guide.actions.revoke"),
    desc: t("admin.subscriptions.guide.actions.revokeDesc"),
  },
]);
...
const allColumns = computed<Column[]>(() => [
  ...
  {
    key: "group",
    label: t("admin.subscriptions.columns.group"),
    sortable: false,
  },
  {
    key: "usage",
    label: t("admin.subscriptions.columns.usage"),
    sortable: false,
  },
  {
    key: "expires_at",
    label: t("admin.subscriptions.columns.expires"),
    sortable: true,
  },
  {
    key: "status",
  ...
  {
    key: "actions",
    label: t("admin.subscriptions.columns.actions"),
    sortable: false,
  },
]);
...
const handleAssignSubscription = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.subscriptions.failedToAssign"),
    );
...
const handleExtendSubscription = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.subscriptions.failedToAdjust"),
    );
...
const confirmRevoke = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.subscriptions.failedToRevoke"),
    );
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail ||
        t("admin.subscriptions.failedToResetQuota"),
    );
...
.usage-amount {
  @apply whitespace-nowrap text-xs tabular-nums text-body;
}

.reset-info {
  @apply flex items-center gap-1 pl-12 text-[10px] text-primary-700;
}
</style>
