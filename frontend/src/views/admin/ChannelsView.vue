  ...
  <AppLayout>
    ...
    >
      <div class="channel-dialog-body">
        <!-- Tab Bar -->
        <div
          class="flex items-center border-b border-hairline flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4"
        >
          <!-- Basic Settings Tab -->
          <button
            type="button"
          ...
          >
            ...
            />
            <span :class="platformTextClass(section.platform)">{{
        ...
        >
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div>
              ...
              <p class="mt-1 ml-6 text-xs text-muted-soft">
                {{
                  t(
              ...
              <div class="flex flex-wrap gap-2">
                ...
                >
                  <input
                    ...
                    class="h-3.5 w-3.5 rounded border-hairline text-primary-600 focus:ring-primary-500"
                    @change="togglePlatform(p)"
                  />
                  <PlatformIcon
                    :platform="p"
                    size="xs"
                    :class="platformTextClass(p)"
                  />
                  <span :class="platformTextClass(p)">{{
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span
                  v-if="section.group_ids.length > 0"
                  class="ml-1 font-normal text-muted-soft"
                >
                  ({{
                    t(
                      "admin.channels.form.selectedCount",
                      { count: section.group_ids.length },
                      `已选 ${section.group_ids.length} 个`,
                    )
                  }})
                </span>
              </label>
              <div
                class="max-h-40 overflow-auto rounded-lg border border-hairline bg-surface-soft p-2"
              >
                <div
                  v-if="groupsLoading"
                ...
                >
                  {{
                    t(
                      "admin.channels.form.noGroupsAvailable",
                      "No groups available",
                    )
                  }}
                </div>
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-hairline px-2 py-1 text-xs transition-colors hover:bg-surface-soft"
                    :class="[
                      section.group_ids.includes(group.id)
                        ? 'bg-primary-50 border-primary-300 '
                        : '',
                      isGroupInOtherChannel(group.id, section.platform)
                        ? 'opacity-40'
                        : '',
                    ]"
                  >
                    <input
                      ...
                      :disabled="
                        isGroupInOtherChannel(group.id, section.platform)
                      "
                      class="h-3 w-3 rounded border-hairline text-primary-600 focus:ring-primary-500"
                    ...
                    <span
                      ...
                      class="text-[10px] text-muted-soft"
            ...
            <div
              v-if="section.platform === 'anthropic' && webSearchGlobalEnabled"
              class="border-t border-hairline pt-3"
            >
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="mt-0.5 text-[11px] text-error">
            ...
            <div>
              ...
              <div
                ...
                class="rounded border border-dashed border-hairline p-2 text-center text-xs text-muted-soft"
              >
                {{
                  t(
              ...
              <div v-else class="space-y-1">
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded p-0.5 text-muted-soft hover:text-error"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
            ...
            <div class="mt-4 border-t border-hairline pt-4 space-y-3">
              <div class="flex items-center justify-between">
              ...
              <p
                ...
                class="text-xs italic text-muted-soft"
              >
              ...
              <div
                ...
                :key="ruleIndex"
                class="space-y-3 rounded-lg border border-hairline p-4"
              >
                <div class="flex items-center justify-between">
                  <input
                    v-model="rule.name"
                    :placeholder="t('admin.channels.form.ruleName')"
                    class="bg-transparent text-sm font-medium text-body placeholder-muted-soft outline-none"
                  />
                  <button
                    type="button"
                    @click="removeAccountStatsRule(sIdx, ruleIndex)"
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      <input
                        ...
                        @change="
                          rule.group_ids.includes(gid)
                            ? rule.group_ids.splice(
                                rule.group_ids.indexOf(gid),
                                1,
                              )
                      ...
                      <span
                        :class="[
                          'font-medium',
                          platformTextClass(section.platform),
                        ]"
                        >{{ getGroupNameById(gid) }}</span
                      >
                    </label>
                  </div>
                  <p
                    v-if="section.group_ids.length === 0"
                    class="mt-1 text-xs text-muted-soft"
                  >
                ...
                <div>
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      <span
                        ...
                        >{{ getRuleAccountLabel(accountId) }}</span
                      >
                      <button
                        type="button"
                        @click="removeRuleAccount(rule, accountId)"
                        class="text-muted-soft hover:text-error"
                      >
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    <div
                      ...
                      "
                      class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border bg-canvas shadow-card"
                    >
                      <button
                        v-for="account in ruleAccountSearchResults[
                          `${section.platform}-${ruleIndex}`
                        ]"
                        :key="account.id"
                        type="button"
                      ...
                      >
                        <span :class="platformTextClass(account.platform)">{{
                          account.name
                        }}</span>
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      @click="addRulePricingEntry(sIdx, ruleIndex)"
                      class="text-xs text-primary-600 hover:text-primary-700"
                    >
                      + {{ t("common.add") }}
                    </button>
...
const columns = computed<Column[]>(() => [
  {
    key: "name",
    label: t("admin.channels.columns.name", "Name"),
    sortable: true,
  },
  {
    key: "description",
  ...
  {
    key: "status",
    label: t("admin.channels.columns.status", "Status"),
    sortable: true,
  },
  {
    key: "group_count",
    label: t("admin.channels.columns.groups", "Groups"),
    sortable: false,
  },
  {
    key: "pricing_count",
  ...
  {
    key: "actions",
    label: t("admin.channels.columns.actions", "Actions"),
    sortable: false,
  },
]);
...
const billingModelSourceOptions = computed(() => [
  {
    ...
    label: t(
      "admin.channels.form.billingModelSourceChannelMapped",
  ...
  },
  {
    value: "requested",
    label: t(
      "admin.channels.form.billingModelSourceRequested",
  ...
  },
  {
    value: "upstream",
    label: t(
      "admin.channels.form.billingModelSourceUpstream",
...
function addRulePricingEntry(sectionIdx: number, ruleIndex: number) {
  form.platforms[sectionIdx].account_stats_pricing_rules[
    ruleIndex
  ].pricing.push({
    models: [],
    billing_mode: "token",
...
function removeRulePricingEntry(
  sectionIdx: number,
  ruleIndex: number,
  pricingIndex: number,
) {
...
function onRuleAccountSearchInput(platform: string, ruleIndex: number) {
  ...
  ruleAccountSearchRunner.trigger(
    key,
...
function formToAPI(): {
  ...
  const featuresConfig: Record<string, unknown> = editingChannel.value
    ?.features_config
...
function apiToForm(channel: Channel): PlatformSection[] {
  ...
  for (const p of Object.keys(channel.model_mapping || {})) {
    if (platformOrder.includes(p as GroupPlatform))
...
async function handleSubmit() {
  ...
  if (!form.name.trim()) {
    appStore.showError(
      t("admin.channels.nameRequired", "Please enter a channel name"),
    );
    return;
  }
    ...
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(
        t(
          ...
          `${platformLabel} 平台未选择分组，请至少选择一个分组或禁用该平台`,
        ),
      );
      ...
      if (entry.models.length === 0) {
        ...
        appStore.showError(
          t(
            ...
            { platform: platformLabel },
            `${platformLabel} 平台下有定价条目未添加模型，请添加模型或删除该条目`,
          ),
        );
        activeTab.value = section.platform;
        return;
  ...
  const { group_ids, model_pricing, model_mapping, features_config } =
    formToAPI();
  ...
  try {
    if (editingChannel.value) {
      const req: UpdateChannelRequest = {
        ...
        model_pricing,
        model_mapping:
          Object.keys(model_mapping).length > 0 ? model_mapping : {},
      ...
      appStore.showSuccess(
        t("admin.channels.updateSuccess", "Channel updated"),
      );
    } else {
      ...
      appStore.showSuccess(
        t("admin.channels.createSuccess", "Channel created"),
      );
    }
    closeDialog();
    loadChannels();
...
async function confirmDelete() {
  ...
  try {
    ...
    appStore.showError(
      extractApiErrorMessage(
        error,
        t("admin.channels.deleteError", "Failed to delete channel"),
      ),
    );
  }
}
...
.channel-tab-inactive {
  @apply border-transparent text-muted hover:text-body hover:border-hairline;
}
</style>
