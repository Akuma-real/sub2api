      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group_count="{ row }">
            <span
              class="inline-flex items-center rounded bg-surface-card px-2 py-0.5 text-xs font-medium text-ink"
            >
      ...
      <div class="channel-dialog-body">
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            <!-- Name -->
            <div>
              <label class="input-label">{{ t('admin.channels.form.name', 'Name') }} <span class="text-error">*</span></label>
              <input
                v-model="form.name"
                type="text"
                required
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                {{ t('admin.channels.form.groups', 'Associated Groups') }} <span class="text-error">*</span>
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-muted-soft">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }, `已选 ${section.group_ids.length} 个`) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-lg border border-hairline bg-cream p-2">
                <div v-if="groupsLoading" class="py-2 text-center text-xs text-muted">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="py-2 text-center text-xs text-muted">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-hairline px-2 py-1 text-xs transition-colors hover:bg-cream"
                    :class="[
                  ...
                  >
                    ...
                    <span class="text-[10px] text-muted-soft">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="border-t border-hairline pt-3">
              <div class="flex items-center justify-between">
                <div>
            ...
            <div v-if="section.platform === 'openai'" class="border-t border-hairline pt-3">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-body">
                    {{ t('admin.channels.form.codexImageGenerationBridge') }}
                  </label>
                  <p class="mt-0.5 text-[11px] text-accent-amber">
            ...
            <div v-if="section.platform === 'anthropic'" class="border-t border-hairline pt-3">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-body">
              ...
              <div class="mb-1 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    class="text-xs text-muted hover:text-primary-600 disabled:opacity-50"
                  >
              ...
              >
                <div class="flex items-center justify-between">
                  <input
                    ...
                    class="bg-transparent text-sm font-medium text-body placeholder-gray-400 outline-none"
                  />
                  <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="text-xs text-error hover:text-error">
                    {{ t('common.delete') }}
                  </button>
                </div>

                <div>
                  <label class="text-xs text-muted">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :class="rule.group_ids.includes(gid)
                        ...
                        : 'border-hairline hover:bg-cream'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-hairline text-primary-600 focus:ring-primary-500" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-muted-soft">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="text-xs text-muted">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      ...
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="text-muted-soft hover:text-error">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    >
                      ...
                      >
                        ...
                        <span class="ml-2 text-xs text-muted-soft">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                  <p class="mt-1 text-xs text-muted-soft">
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-muted">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs text-primary-600 hover:text-primary-700">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-hairline p-2 text-center text-xs text-muted-soft">
