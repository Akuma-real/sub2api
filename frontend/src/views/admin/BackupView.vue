      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            class="input w-full"
            :placeholder="
              s3SecretConfigured ? t('admin.backup.s3.secretConfigured') : ''
            "
          />
        </div>
        <label
          class="inline-flex items-center gap-2 text-sm text-body md:col-span-2"
        >
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        ...
        >
          {{
            testingS3
              ? t("common.loading")
              : t("admin.backup.s3.testConnection")
          }}
        </button>
        <button
          type="button"
    ...
    <div class="card p-6">
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[800px] text-sm">
          <thead>
            <tr
              class="border-b border-hairline text-left text-xs uppercase tracking-wide text-muted"
            >
          ...
          <tbody>
            <tr
              ...
              :key="record.id"
              class="border-b border-hairline-soft align-top"
            >
            ...
            <tr v-if="backups.length === 0">
              <td colspan="8" class="py-6 text-center text-sm text-muted">
      ...
      >
        ...
        >
          ...
          <div class="mb-5">
            ...
            <div class="mt-2 rounded-lg bg-accent-amber/15 p-3 text-xs text-warning">
          ...
          <div class="mb-5">
            ...
            <p class="ml-8 text-sm text-body">
              {{ t("admin.backup.r2Guide.step3.desc") }}
            </p>
            <code
              class="ml-8 mt-1 block rounded bg-surface-card px-3 py-2 text-xs text-body-strong"
              >https://&lt;{{
                t("admin.backup.r2Guide.step3.accountId")
              }}&gt;.r2.cloudflarestorage.com</code
            >
          </div>
          ...
          <div class="mb-5">
            ...
            <div class="ml-8 overflow-hidden rounded-lg border border-hairline">
              <table class="w-full text-sm">
                <tbody>
                  <tr
                    v-for="(row, i) in r2ConfigRows"
                    :key="i"
          ...
          <!-- Free tier note -->
          <div class="rounded-lg bg-success/15 p-3 text-xs text-success">
    ...
    try {
      ...
      if (record.status === "completed" || record.status === "failed") {
        ...
        if (record.status === "completed") {
          ...
          appStore.showError(
            record.error_message || t("admin.backup.operations.backupFailed"),
          );
        }
        await loadBackups();
      }
    ...
    try {
      ...
      if (
        ...
        if (record.restore_status === "completed") {
          ...
          appStore.showError(
            record.restore_error || t("admin.backup.operations.restoreFailed"),
          );
        }
        await loadBackups();
      }
...
function handleVisibilityChange() {
  if (document.hidden) {
    ...
    loadBackups().then(() => {
      ...
      const restoring = backups.value.find(
        (r) => r.restore_status === "running",
      );
      if (restoring) {
...
const r2ConfigRows = computed(() => [
  {
    field: t("admin.backup.s3.endpoint"),
    value: "https://<account_id>.r2.cloudflarestorage.com",
  },
  { field: t("admin.backup.s3.region"), value: "auto" },
  {
    field: t("admin.backup.s3.bucket"),
    value: t("admin.backup.r2Guide.step4.bucketValue"),
  },
  { field: t("admin.backup.s3.prefix"), value: "backups/" },
  { field: "Access Key ID", value: t("admin.backup.r2Guide.step4.fromStep2") },
  {
    field: "Secret Access Key",
    value: t("admin.backup.r2Guide.step4.fromStep2"),
  },
  {
    field: t("admin.backup.s3.forcePathStyle"),
    value: t("admin.backup.r2Guide.step4.unchecked"),
  },
]);
...
async function loadS3Config() {
  try {
    ...
    appStore.showError(
      (error as { message?: string })?.message || t("errors.networkError"),
    );
  }
}
...
async function createBackup() {
  creatingBackup.value = true;
  try {
    const record = await adminAPI.backup.createBackup({
      expire_days: manualExpireDays.value,
    });
    // 插入到列表顶部
...
onMounted(async () => {
  ...
  const restoringBackup = backups.value.find(
    (r) => r.restore_status === "running",
  );
  if (restoringBackup) {
