<template>
  <component
    ...
    :class="
      isFullscreen
        ? 'flex min-h-screen flex-col justify-center bg-surface-soft '
        : ''
    "
  >
    <div :class="[isFullscreen ? 'p-4 md:p-6' : '', 'space-y-6 pb-12']">
      <div
        v-if="errorMessage"
        class="rounded-lg bg-error/15 p-4 text-sm text-error"
      >
        {{ errorMessage }}
      </div>
      ...
      <div
        v-if="
          opsEnabled && showOpenAITokenStats && !(loading && !hasLoadedOnce)
        "
...
} from "./components/OpsRequestDetailsModal.vue";
...
async function refreshSwitchTrendWithCancel(
  fetchSeq: number,
  signal: AbortSignal,
) {
  if (!opsEnabled.value) return;
  ...
  try {
    const data = await opsAPI.getThroughputTrend(buildSwitchTrendParams(), {
      signal,
    });
    if (fetchSeq !== dashboardFetchSeq) return;
...
async function refreshThroughputTrendWithCancel(
  ...
  try {
    ...
    appStore.showError(
      err?.message || t("admin.ops.failedToLoadThroughputTrend"),
    );
  } finally {
    if (fetchSeq === dashboardFetchSeq) {
...
async function refreshCoreSnapshotWithCancel(
  ...
  try {
    const data = await opsAPI.getDashboardSnapshotV2(buildApiParams(), {
      signal,
    });
    if (fetchSeq !== dashboardFetchSeq) return;
...
async function refreshLatencyHistogramWithCancel(
  ...
  try {
    ...
    appStore.showError(
      err?.message || t("admin.ops.failedToLoadLatencyHistogram"),
    );
  } finally {
    if (fetchSeq === dashboardFetchSeq) {
...
async function refreshErrorDistributionWithCancel(
  ...
  try {
    ...
    appStore.showError(
      err?.message || t("admin.ops.failedToLoadErrorDistribution"),
    );
  } finally {
    if (fetchSeq === dashboardFetchSeq) {
