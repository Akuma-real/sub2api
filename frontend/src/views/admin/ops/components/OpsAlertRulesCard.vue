...
async function load() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.alertRules.loadFailed"),
    );
    rules.value = [];
  } finally {
    loading.value = false;
...
const groupOptions = computed<SelectOption[]>(() => {
  ...
  return [
    { value: null, label: t("admin.ops.alertRules.form.allGroups") },
    ...groupOptionsBase.value,
  ];
});
...
const metricDefinitions = computed(() => {
  return [
    ...
    },
    {
      ...
      description: t(
        "admin.ops.alertRules.metricDescriptions.accountTempUnscheduledCount",
      ),
      recommendedOperator: ">",
      recommendedThreshold: 0,
    },
...
const editorValidation = computed(() => {
  ...
  if (!r.metric_type)
    errors.push(t("admin.ops.alertRules.validation.metricRequired"));
  if (
  ...
  if (!r.operator)
    errors.push(t("admin.ops.alertRules.validation.operatorRequired"));
...
async function save() {
  ...
  if (!editorValidation.value.valid) {
    appStore.showError(
      editorValidation.value.errors[0] ||
        t("admin.ops.alertRules.validation.invalid"),
    );
    return;
  }
  saving.value = true;
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.alertRules.saveFailed"),
    );
  } finally {
    saving.value = false;
  }
...
async function confirmDelete() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.alertRules.deleteFailed"),
    );
  }
}
...
<template>
  <div class="rounded-lg bg-canvas p-6 shadow-sm ring-1 ring-ink/5">
    ...
    <div
      v-else
      class="max-h-[520px] overflow-hidden rounded-xl border border-hairline"
    >
      <div class="max-h-[520px] overflow-y-auto">
        <table class="min-w-full divide-y divide-hairline">
          ...
          <tbody class="divide-y divide-hairline bg-canvas">
            ...
            >
              <td class="px-4 py-3">
                <div class="text-xs font-bold text-ink">{{ row.name }}</div>
                <div
                ...
                <div
                  v-if="row.updated_at"
                  class="mt-1 text-[10px] text-muted-soft"
                >
              ...
              <td class="whitespace-nowrap px-4 py-3 text-xs text-body">
                ...
                <span class="mx-1 text-muted-soft">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td
                class="whitespace-nowrap px-4 py-3 text-xs font-bold text-body"
              >
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-body">
    ...
    >
      <div class="space-y-4">
        <div
          ...
          class="rounded-xl bg-error/15 p-4 text-xs text-error"
        >
          <div class="font-bold">
            {{ t("admin.ops.alertRules.validation.title") }}
          </div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            <label class="input-label">
              ...
              <span v-if="isGroupMetricSelected" class="ml-1 text-error"
                >*</span
              >
            </label>
            <Select
            ...
            <p class="mt-1 text-xs text-muted">
              {{
                isGroupMetricSelected
                  ...
                  : t("admin.ops.alertRules.hints.groupOptional")
              }}
            </p>
          </div>
          ...
          <div
            class="flex items-center justify-between rounded-xl bg-surface-soft px-4 py-3 md:col-span-2"
          >
