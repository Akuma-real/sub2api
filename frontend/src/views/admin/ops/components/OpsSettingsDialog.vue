...
async function loadAllSettings() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.settings.loadFailed"),
    );
  } finally {
    loading.value = false;
  }
...
const validation = computed(() => {
  ...
  if (advancedSettings.value) {
    ...
    if (
      minute_metrics_retention_days < 0 ||
      minute_metrics_retention_days > 365
    ) {
      errors.push(t("admin.ops.settings.validation.retentionDaysRange"));
    }
    if (
      hourly_metrics_retention_days < 0 ||
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.message ||
        ...
        t("admin.ops.settings.saveFailed"),
    );
  } finally {
    saving.value = false;
  }
  ...
  >
    ...
    >
      ...
      <div
        ...
        class="rounded-lg border border-accent-amber/30 bg-accent-amber/15 p-3 text-xs text-warning"
      >
        <div class="font-bold">
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div>
          <label class="input-label">{{
            t("admin.ops.settings.evaluationInterval")
          }}</label>
          <input
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div class="space-y-4">
          ...
          <div v-if="emailConfig.alert.enabled">
            <label class="input-label">{{
              t("admin.ops.settings.alertRecipients")
            }}</label>
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span
                ...
                :key="email"
                class="inline-flex items-center gap-2 rounded-full bg-accent-teal/15 px-3 py-1 text-xs font-medium text-primary-700"
              >
                {{ email }}
                <button
                  type="button"
                  class="text-primary-700/80 hover:text-ink"
          ...
          <div v-if="emailConfig.alert.enabled">
            <label class="input-label">{{
              t("admin.ops.settings.minSeverity")
            }}</label>
            <Select
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div class="space-y-4">
          ...
          <div v-if="emailConfig.report.enabled">
            <label class="input-label">{{
              t("admin.ops.settings.reportRecipients")
            }}</label>
            <div class="flex gap-2">
              <input
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div class="space-y-4">
          <div>
            <label class="input-label">{{
              t("admin.ops.settings.slaMinPercent")
            }}</label>
            <input
          ...
          <div>
            <label class="input-label">{{
              t("admin.ops.settings.ttftP99MaxMs")
            }}</label>
            <input
          ...
          <div>
            <label class="input-label">{{
              t("admin.ops.settings.requestErrorRateMaxPercent")
            }}</label>
            <input
          ...
          <div>
            <label class="input-label">{{
              t("admin.ops.settings.upstreamErrorRateMaxPercent")
            }}</label>
            <input
      ...
      <!-- 高级设置 -->
      <details class="rounded-lg bg-surface-soft">
        <summary class="cursor-pointer p-4 text-sm font-semibold text-ink">
          {{ t("admin.ops.settings.advancedSettings") }}
        </summary>
        <div class="space-y-4 px-4 pb-4">
          <!-- 数据保留策略 -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-body">
            ...
            <div v-if="advancedSettings.data_retention.cleanup_enabled">
              <label class="input-label">{{
                t("admin.ops.settings.cleanupSchedule")
              }}</label>
              <input
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div>
                <label class="input-label">{{
                  t("admin.ops.settings.errorLogRetentionDays")
                }}</label>
                <input
                  v-model.number="
              ...
              <div>
                <label class="input-label">{{
                  t("admin.ops.settings.minuteMetricsRetentionDays")
                }}</label>
                <input
                  v-model.number="
              ...
              <div>
                <label class="input-label">{{
                  t("admin.ops.settings.hourlyMetricsRetentionDays")
                }}</label>
                <input
                  v-model.number="
                    advancedSettings.data_retention
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.ignoreCountTokensErrors")
                }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.ignoreContextCanceled")
                }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.ignoreNoAvailableAccounts")
                }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.ignoreInvalidApiKeyErrors")
                }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.ignoreInsufficientBalanceErrors")
                }}</label>
                <p class="mt-1 text-xs text-muted">
                  {{
          ...
          <div class="space-y-3">
            ...
            <div v-if="advancedSettings.auto_refresh_enabled">
              <label class="input-label">{{
                t("admin.ops.settings.refreshInterval")
              }}</label>
              <Select
                ...
                :options="[
                  {
                    value: 15,
                    label: t('admin.ops.settings.refreshInterval15s'),
                  },
                  {
                    value: 30,
                    label: t('admin.ops.settings.refreshInterval30s'),
                  },
                  {
                    value: 60,
                    label: t('admin.ops.settings.refreshInterval60s'),
                  },
                ]"
              />
            </div>
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.displayAlertEvents")
                }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-body">{{
                  t("admin.ops.settings.displayOpenAITokenStats")
                }}</label>
                <p class="mt-1 text-xs text-muted">
