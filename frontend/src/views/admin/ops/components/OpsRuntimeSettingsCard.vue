...
function validateRuntimeSettings(
  ...
  if (thresholds) {
    ...
    if (thresholds.upstream_error_rate_percent_max != null) {
      if (
        ...
        errors.push(
          t("admin.ops.runtime.validation.upstreamErrorRateMaxRange"),
        );
      }
    }
  }
  ...
  if (lock?.enabled) {
    if (!lock.key || lock.key.trim().length < 3) {
      ...
      errors.push(
        t("admin.ops.runtime.validation.lockKeyPrefix", { prefix: "ops: " }),
      );
    }
    if (
  ...
  if (silencing?.enabled) {
    ...
    for (let idx = 0; idx < entries.length; idx++) {
      ...
      if (!untilEntry) {
        errors.push(
          t("admin.ops.runtime.silencing.entries.validation.untilRequired"),
        );
        break;
      }
      ...
      if (!Number.isFinite(parsedEntry)) {
        errors.push(
          t("admin.ops.runtime.silencing.entries.validation.untilFormat"),
        );
        break;
      }
      ...
      if (
        ...
        errors.push(
          t("admin.ops.runtime.silencing.entries.validation.ruleIdPositive"),
        );
        break;
      }
...
async function loadSettings() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.runtime.loadFailed"),
    );
  } finally {
    loading.value = false;
  }
...
async function saveAlertSettings() {
  ...
  if (!alertValidation.value.valid) {
    appStore.showError(
      alertValidation.value.errors[0] ||
        t("admin.ops.runtime.validation.invalid"),
    );
    return;
  }
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.runtime.saveFailed"),
    );
  } finally {
    saving.value = false;
  }
...
<template>
  <div class="rounded-lg bg-canvas p-6 shadow-sm ring-1 ring-ink/5">
    ...
    <div v-if="!alertSettings" class="text-sm text-muted">
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div
            ...
            "
            class="text-xs text-body md:col-span-2"
          >
            {{ t("admin.ops.runtime.silencing.globalUntil") }}:
            <span class="ml-1 font-mono text-ink">{{
              alertSettings.silencing.global_until_rfc3339
            }}</span>
          </div>
          ...
          <details class="col-span-1 md:col-span-2">
            <summary
              class="cursor-pointer text-xs font-medium text-primary-700 hover:text-primary-700"
            >
              {{ t("admin.ops.runtime.showAdvancedDeveloperSettings") }}
            </summary>
            <div
              class="mt-2 grid grid-cols-1 gap-3 rounded-lg bg-surface-card p-3 md:grid-cols-2"
            >
              <div class="text-xs text-muted">
                {{ t("admin.ops.runtime.lockEnabled") }}:
                <span class="ml-1 font-mono text-body">{{
  ...
  >
    <div v-if="draftAlert" class="space-y-4">
      ...
      <div>
        <div class="mb-1 text-xs font-medium text-body">
          {{ t("admin.ops.runtime.evalIntervalSeconds") }}
        </div>
        <input
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <label class="inline-flex items-center gap-2 text-sm text-body">
          <input
            ...
            type="checkbox"
            class="h-4 w-4 rounded border-hairline"
          />
        ...
        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          ...
          <div class="rounded-xl border border-hairline bg-canvas p-4">
            ...
            <div
              ...
              class="mt-3 rounded-lg bg-surface-soft p-3 text-xs text-muted"
            >
            ...
            <div v-else class="mt-4 space-y-4">
              ...
              >
                <div class="mb-3 flex items-center justify-between">
                  <div class="text-xs font-bold text-ink">
                    {{
                      t("admin.ops.runtime.silencing.entries.entryTitle", {
                        n: idx + 1,
                      })
                    }}
                  </div>
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      :placeholder="
                        t(
                          'admin.ops.runtime.silencing.entries.ruleIdPlaceholder',
                        )
                      "
                      @input="
                  ...
                  <div>
                    ...
                    <input
                      ...
                      :placeholder="
                        t(
                          'admin.ops.runtime.silencing.entries.severitiesPlaceholder',
                        )
                      "
                      @input="
      ...
      <details class="rounded-lg border border-hairline bg-surface-soft p-3">
        <summary class="cursor-pointer text-xs font-medium text-body">
          {{ t("admin.ops.runtime.advancedSettingsSummary") }}
        </summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="inline-flex items-center gap-2 text-xs text-body">
              <input
          ...
          <div class="md:col-span-2">
            ...
            >
              {{
                t("admin.ops.runtime.validation.lockKeyHint", {
                  prefix: "ops: ",
                })
              }}
            </p>
