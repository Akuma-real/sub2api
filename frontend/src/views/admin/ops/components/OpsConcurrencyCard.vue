...
const userRows = computed((): UserRow[] => {
  ...
  return Object.keys(userStats)
    ...
    .sort(
      (a, b) =>
        b.current_in_use - a.current_in_use ||
        b.load_percentage - a.load_percentage,
    );
});
...
const displayTitle = computed(() => {
  if (displayDimension.value === "user")
    return t("admin.ops.concurrency.byUser");
  if (displayDimension.value === "account")
    return t("admin.ops.concurrency.byAccount");
  if (displayDimension.value === "group")
    return t("admin.ops.concurrency.byGroup");
...
async function loadData() {
  ...
  try {
    ...
    errorMessage.value =
      err?.response?.data?.detail || t("admin.ops.concurrency.loadFailed");
  } finally {
    loading.value = false;
  }
}
...
function getLoadBarClass(loadPct: number): string {
  if (loadPct >= 90) return "bg-error ";
  if (loadPct >= 70) return "bg-accent-amber ";
  if (loadPct >= 50) return "bg-accent-amber ";
  return "bg-success ";
}
...
function getLoadTextClass(loadPct: number): string {
  if (loadPct >= 90) return "text-error ";
...
<template>
  <div
    class="flex h-full flex-col rounded-lg bg-canvas p-6 shadow-sm ring-1 ring-ink/5"
  >
    <!-- 头部 -->
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="
            showByUser
              ...
              : 'bg-surface-card text-muted hover:bg-hairline hover:text-body '
          "
          :title="
            showByUser
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-surface-card px-2 py-1 text-[11px] font-semibold text-body transition-colors hover:bg-hairline disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="loading"
    ...
    <div
      v-if="errorMessage"
      class="mb-3 shrink-0 rounded-xl bg-error/15 p-2.5 text-xs text-error"
    >
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-hairline text-sm text-muted"
    >
    ...
    <div
      v-else
      class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-hairline"
    >
      <!-- 维度标题栏 -->
      <div
        class="flex shrink-0 items-center justify-between border-b border-hairline bg-surface-soft px-3 py-2"
      >
      ...
      <!-- 用户视图 -->
      <div
        v-else-if="displayDimension === 'user'"
        class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3"
      >
        <div
          v-for="row in displayRows as UserRow[]"
          :key="row.key"
          class="rounded-lg bg-surface-soft p-2.5"
        >
          <!-- 用户信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span
                class="truncate text-[11px] font-bold text-ink"
              ...
              <span
                v-if="row.username"
                class="shrink-0 truncate text-[10px] text-muted-soft"
            ...
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              ...
              <span
                :class="['font-bold', getLoadTextClass(row.load_percentage)]"
              >
          ...
          <!-- 进度条 -->
          <div class="h-1.5 w-full overflow-hidden rounded-full bg-hairline">
            <div
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            ...
            >
              {{
                t("admin.ops.concurrency.queued", {
                  count: row.waiting_in_queue,
                })
              }}
            </span>
          </div>
      ...
      >
        ...
        >
          ...
          <!-- 进度条 -->
          <div
            class="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-hairline"
          >
            <div
              class="h-full rounded-full transition-all duration-300"
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <span
              ...
              class="rounded-full bg-accent-amber/15 px-1.5 py-0.5 font-semibold text-warning"
            >
              {{
                t("admin.ops.concurrency.rateLimited", {
                  count: row.rate_limited_accounts,
                })
              }}
            </span>
            ...
            <span
              ...
              class="rounded-full bg-error/15 px-1.5 py-0.5 font-semibold text-error"
            >
              {{
                t("admin.ops.concurrency.errorAccounts", {
                  count: row.error_accounts,
                })
              }}
            </span>
            ...
            <span
              ...
              class="rounded-full bg-primary-100 px-1.5 py-0.5 font-semibold text-primary-700"
            >
              {{
                t("admin.ops.concurrency.queued", {
                  count: row.waiting_in_queue,
        ...
        >
          ...
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-muted-soft">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-ink">
                {{ row.current_in_use }}/{{ row.max_capacity }}
              </span>
              <!-- 状态徽章 -->
              <span
                v-if="row.is_available"
                class="inline-flex items-center gap-1 rounded bg-success/15 px-1.5 py-0.5 text-[10px] font-medium text-success"
              >
                <svg
                  class="h-3 w-3"
                  fill="none"
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-accent-amber/15 px-1.5 py-0.5 text-[10px] font-medium text-warning"
              >
                <svg
                  class="h-3 w-3"
                  fill="none"
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-error/15 px-1.5 py-0.5 text-[10px] font-medium text-error"
              >
                <svg
                  class="h-3 w-3"
                  fill="none"
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-surface-card px-1.5 py-0.5 text-[10px] font-medium text-body"
              >
...
.custom-scrollbar {
  ...
  scrollbar-color: rgba(142, 139, 130, 0.3) transparent;
}
...
.custom-scrollbar::-webkit-scrollbar-thumb {
  background-color: rgba(142, 139, 130, 0.3);
  border-radius: 3px;
}

.custom-scrollbar::-webkit-scrollbar-thumb:hover {
  background-color: rgba(142, 139, 130, 0.5);
}
</style>
