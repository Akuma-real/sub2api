...
async function loadConfig() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.email.loadFailed"),
    );
  } finally {
    loading.value = false;
  }
...
async function saveConfig() {
  ...
  if (!editorValidation.value.valid) {
    appStore.showError(
      editorValidation.value.errors[0] ||
        t("admin.ops.email.validation.invalid"),
    );
    return;
  }
  saving.value = true;
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.email.saveFailed"),
    );
  } finally {
    saving.value = false;
  }
...
function validateCronField(enabled: boolean, cron: string): string | null {
  ...
  if (!cron || !cron.trim())
    return t("admin.ops.email.validation.cronRequired");
  if (cron.trim().split(/\s+/).length < 5)
    return t("admin.ops.email.validation.cronFormat");
  return null;
}
...
const editorValidation = computed(() => {
  ...
  if (invalidAlertRecipients.length > 0)
  ...
  if (invalidReportRecipients.length > 0)
    errors.push(t("admin.ops.email.validation.invalidRecipients"));
...
<template>
  <div class="rounded-lg bg-canvas p-6 shadow-sm ring-1 ring-ink/5">
    ...
    <div v-if="!config" class="text-sm text-muted">
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            ...
            <p v-if="alertRecipientError" class="mt-1 text-xs text-error">
              {{ alertRecipientError }}
            </p>
      ...
      <div class="rounded-lg bg-surface-soft p-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            ...
            <p v-if="reportRecipientError" class="mt-1 text-xs text-error">
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-body">
                  {{ t("admin.ops.email.dailySummary") }}
                </div>
                <div class="flex items-center gap-2">
                  <label
                    class="inline-flex items-center gap-2 text-sm text-body"
                  >
                    <input
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-body">
                  {{ t("admin.ops.email.weeklySummary") }}
                </div>
                <div class="flex items-center gap-2">
                  <label
                    class="inline-flex items-center gap-2 text-sm text-body"
              ...
              <div>
                <div class="mb-1 text-xs font-medium text-body">
                  {{ t("admin.ops.email.accountHealth") }}
                </div>
                <div class="flex items-center gap-2">
                  <label
                    class="inline-flex items-center gap-2 text-sm text-body"
