<template>
  ...
  >
    ...
    <div v-else-if="!detail" class="py-10 text-center text-sm text-muted">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-surface-soft p-4">
          ...
          <div class="mt-1 break-all font-mono text-sm font-medium text-ink">
            {{ requestId || "—" }}
          </div>
        </div>
        ...
        <div v-if="detail.api_key_prefix" class="rounded-xl bg-surface-soft p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-muted-soft">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm font-medium text-ink">
            {{ detail.api_key_prefix }}
          </div>
        </div>

        <div v-if="detail.attempted_key_prefix" class="rounded-xl bg-surface-soft p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-muted-soft">{{ t('admin.ops.errorDetail.attemptedKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm font-medium text-ink">
            {{ detail.attempted_key_prefix }}
          </div>
        </div>

        <div v-if="detail.deleted_key_owner_email" class="rounded-xl bg-surface-soft p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-muted-soft">{{ t('admin.ops.errorDetail.deletedKeyOwner') }}</div>
          <div class="mt-1 text-sm font-medium text-ink">
            {{ detail.deleted_key_owner_email }}
            <span v-if="detail.deleted_key_name" class="ml-1 text-xs text-muted">({{ detail.deleted_key_name }})</span>
            <span class="ml-2 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold ring-1 ring-inset bg-error/15 text-error ring-error/25">
      ...
      <div class="rounded-xl bg-surface-soft p-6">
        <h3 class="text-sm font-black uppercase tracking-wider text-ink">
          {{ t("admin.ops.errorDetail.responseBody") }}
        </h3>
        <pre
          class="mt-4 max-h-[520px] overflow-auto rounded-xl border border-hairline bg-canvas p-4 text-xs text-body-strong"
        ><code>{{ prettyJSON(primaryResponseBody || '') }}</code></pre>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-xl bg-surface-soft p-6">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-ink">
            {{ t("admin.ops.errorDetails.upstreamErrors") }}
          </h3>
          <div class="text-xs text-muted" v-if="correlatedUpstreamLoading">
            {{ t("common.loading") }}
          </div>
        </div>
        ...
        <div v-else class="mt-4 space-y-3">
          <div
            ...
            :key="ev.id"
            class="rounded-xl border border-hairline bg-canvas p-4"
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-ink">
                #{{ idx + 1 }}
                <span
                  v-if="ev.type"
                  class="ml-2 rounded-md bg-surface-card px-2 py-0.5 font-mono text-[10px] font-bold text-body"
                  >{{ ev.type }}</span
                >
              </div>
            ...
            <div
              class="mt-3 grid grid-cols-1 gap-2 text-xs text-body sm:grid-cols-2"
            >
              <div>
                <span class="text-muted-soft"
                  >{{ t("admin.ops.errorDetail.upstreamEvent.status") }}:</span
                >
            ...
            <div
              v-if="ev.message"
              class="mt-3 break-words text-sm font-medium text-ink"
            >
              {{ ev.message }}
            </div>
            ...
            <pre
              ...
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-hairline bg-surface-soft p-3 text-xs text-body-strong"
