...
const levelBadgeClass = (level: string) => {
  ...
  if (v === "error" || v === "fatal") return "bg-error/15 text-error ";
  if (v === "warn" || v === "warning") return "bg-accent-amber/15 text-warning ";
  if (v === "debug") return "bg-surface-soft text-body ";
...
<template>
  <section class="rounded-lg border border-hairline bg-canvas p-4 shadow-sm">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-ink">系统日志</h3>
        <p class="mt-1 text-xs text-muted">
          默认按最新时间倒序，支持筛选搜索与按条件清理。
        </p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-surface-card px-2 py-1 text-body"
          >队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span
        >
        <span class="rounded-md bg-surface-card px-2 py-1 text-body"
          >写入 {{ health.written_count }}</span
        >
        <span class="rounded-md bg-accent-amber/15 px-2 py-1 text-warning"
          >丢弃 {{ health.dropped_count }}</span
        >
        <span class="rounded-md bg-error/15 px-2 py-1 text-error"
          >失败 {{ health.write_failed_count }}</span
        >
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-hairline bg-surface-soft p-3">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-body">
          运行时日志配置（实时生效）
        </div>
        <span v-if="runtimeLoading" class="text-xs text-muted">加载中...</span>
      </div>
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-error">
    ...
    <div class="overflow-hidden rounded-xl border border-hairline">
      ...
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-hairline">
          <thead class="bg-surface-soft">
            <tr>
              <th
                class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-muted"
              >
                时间
              </th>
              <th
                class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-muted"
              >
                级别
              </th>
              <th
                class="px-3 py-2 text-left text-[11px] font-semibold text-muted"
              >
                日志详细信息
              </th>
            </tr>
          ...
          <tbody class="divide-y divide-hairline-soft">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              ...
              <td
                class="px-3 py-2 text-xs text-body whitespace-normal break-all"
              >
