...
const healthScoreColor = computed(() => {
  if (isSystemIdle.value) return "#8e8b82"; // gray-400
  const score = healthScoreValue.value;
  if (score == null) return "#8e8b82";
  if (score >= 90) return "#5db872"; // green
  if (score >= 60) return "#e8a55a"; // yellow
  return "#c64545"; // red
});

const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return "text-muted-soft";
  const score = healthScoreValue.value;
...
const diagnosisReport = computed<DiagnosisItem[]>(() => {
  ...
  if (sm) {
    ...
    if (cpuPct > 90) {
      report.push({
        type: "critical",
        message: t("admin.ops.diagnosis.cpuCritical", {
          usage: cpuPct.toFixed(1),
        }),
    ...
    if (memPct > 90) {
      report.push({
        type: "critical",
        message: t("admin.ops.diagnosis.memoryCritical", {
          usage: memPct.toFixed(1),
        }),
      ...
      report.push({
        type: "warning",
        message: t("admin.ops.diagnosis.memoryHigh", {
          usage: memPct.toFixed(1),
        }),
  ...
  if (upstreamRatePct > 5) {
    report.push({
      type: "critical",
      message: t("admin.ops.diagnosis.upstreamCritical", {
        rate: upstreamRatePct.toFixed(2),
      }),
    ...
    report.push({
      type: "warning",
      message: t("admin.ops.diagnosis.upstreamHigh", {
        rate: upstreamRatePct.toFixed(2),
      }),
  ...
  if (errorPct > 3) {
    report.push({
      type: "critical",
      message: t("admin.ops.diagnosis.errorHigh", {
    ...
    report.push({
      type: "warning",
      message: t("admin.ops.diagnosis.errorElevated", {
        rate: errorPct.toFixed(2),
      }),
  ...
  if (healthScoreValue.value != null) {
    if (healthScoreValue.value < 60) {
      report.push({
        type: "critical",
        message: t("admin.ops.diagnosis.healthCritical", {
          score: healthScoreValue.value,
        }),
      ...
      report.push({
        type: "warning",
        message: t("admin.ops.diagnosis.healthLow", {
          score: healthScoreValue.value,
        }),
...
const dbMiddleClass = computed(() => {
  if (systemMetrics.value?.db_ok === false) return "text-error ";
  if (dbUsagePercent.value != null) {
    if (dbUsagePercent.value >= 90) return "text-error ";
...
const redisMiddleClass = computed(() => {
  if (systemMetrics.value?.redis_ok === false) return "text-error ";
  if (redisUsagePercent.value != null) {
    if (redisUsagePercent.value >= 90) return "text-error ";
...
<template>
  <div
    :class="[
      'flex flex-col gap-4 rounded-lg bg-canvas shadow-sm ring-1 ring-ink/5 ',
      props.fullscreen ? 'p-8' : 'p-6',
    ]"
  >
    <!-- Top Toolbar -->
    <div
      class="flex flex-wrap items-center justify-between gap-4 border-b border-hairline-soft pb-4"
    >
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-ink">
          <svg
            class="h-6 w-6 text-accent-teal"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div
          ...
          class="mt-1 flex items-center gap-3 text-xs text-muted"
        >
          <span
          ...
          >
            <span class="relative flex h-2 w-2">
              <span
                ...
                :class="props.loading ? 'bg-muted-soft' : 'bg-success'"
              ></span>
            </span>
            {{
              props.loading ? t("admin.ops.loadingText") : t("admin.ops.ready")
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-hairline sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-card text-muted transition-colors hover:bg-hairline"
          :disabled="loading"
        ...
        <div
          v-if="!props.fullscreen"
          class="mx-1 hidden h-4 w-[1px] bg-hairline sm:block"
        ></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-accent-teal/15 px-3 text-xs font-bold text-primary-700 transition-colors hover:bg-accent-teal/25"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-surface-card px-3 text-xs font-bold text-body transition-colors hover:bg-hairline"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-card text-body transition-colors hover:bg-hairline"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div
        :class="[
          'rounded-lg bg-surface-soft lg:col-span-5',
      ...
      >
        ...
        >
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-canvas/60 md:border-r md:border-hairline md:pr-6"
          >
            ...
            >
              <div
                class="rounded-xl bg-canvas p-4 shadow-card ring-1 ring-ink/5"
              >
                <h4
                  class="mb-3 border-b border-hairline-soft pb-2 text-sm font-bold text-ink flex items-center gap-2"
                >
                  <Icon name="brain" size="sm" class="text-accent-teal" />
                ...
                <div class="space-y-3">
                  ...
                  >
                    ...
                    <div class="flex-1">
                      ...
                      <div
                        v-if="item.action"
                        class="mt-1 text-[11px] text-primary-700 flex items-center gap-1"
                      >
                ...
                <div
                  class="mt-3 border-t border-hairline-soft pt-2 text-[10px] text-muted-soft"
                >
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span
                  :class="[
                    ...
                    'font-bold uppercase tracking-wider text-muted-soft',
                  ]"
                  >{{ t("admin.ops.health") }}</span
                >
              </div>
            </div>
            ...
            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div
                class="flex items-center justify-center gap-1 text-xs font-medium text-muted"
              >
            ...
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <div class="relative flex h-3 w-3 shrink-0">
                  ...
                  ></span>
                  <span
                    class="relative inline-flex h-3 w-3 rounded-full bg-accent-teal"
                  ></span>
                </div>
                <h3
                  class="text-xs font-bold uppercase tracking-wider text-muted-soft"
                >
                  {{ t("admin.ops.realtime.title") }}
                </h3>
                <HelpTooltip
                  v-if="!props.fullscreen"
                  :content="t('admin.ops.tooltips.qps')"
                />
              </div>
                ...
                <button
                  ...
                  :class="
                    realtimeWindow === window
                      ...
                      : 'bg-hairline text-body hover:bg-surface-cream-strong '
                  "
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              ...
              <div>
                <div
                  :class="[
                    props.fullscreen ? 'text-xs' : 'text-[10px]',
                    'font-bold uppercase text-muted-soft',
                  ]"
                >
                  {{ t("admin.ops.current") }}
                </div>
                ...
                <div>
                  ...
                  <div
                    :class="[
                      ...
                      'mt-1 space-y-0.5 font-medium text-body ',
                    ]"
                  >
                    <div class="flex items-baseline gap-1.5">
      ...
      >
        <!-- Card 1: Requests -->
        <div class="rounded-lg bg-surface-soft p-4" style="order: 1">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-muted-soft">{{
                t("admin.ops.requestsTitle")
              }}</span>
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.totalRequests')"
              />
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-accent-teal hover:underline"
              type="button"
              @click="
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              ...
              <span class="font-bold text-ink">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t("admin.ops.tokens") }}:</span>
              <span class="font-bold text-ink">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t("admin.ops.avgQps") }}:</span>
              <span class="font-bold text-ink">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t("admin.ops.avgTps") }}:</span>
              <span class="font-bold text-ink">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-lg bg-surface-soft p-4" style="order: 2">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-muted-soft">{{
                t("admin.ops.sla")
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.sla')"
              />
              <span
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-hairline">
            <div
        ...
        <div class="rounded-lg bg-surface-soft p-4" style="order: 4">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-muted-soft">{{
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.latency')"
              />
            </div>
            <button
              v-if="!props.fullscreen"
          ...
          <div class="mt-2 flex items-baseline gap-2">
            ...
            <span class="text-xs font-bold text-muted-soft">ms (P99)</span>
          </div>
          <div
          ...
          >
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span class="font-bold text-ink">{{ durationP95Ms ?? "-" }}</span>
              <span class="text-muted-soft">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted">P90:</span>
              <span class="font-bold text-ink">{{ durationP90Ms ?? "-" }}</span>
              <span class="text-muted-soft">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted">P50:</span>
              <span class="font-bold text-ink">{{ durationP50Ms ?? "-" }}</span>
              <span class="text-muted-soft">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted">Avg:</span>
              <span class="font-bold text-ink">{{ durationAvgMs ?? "-" }}</span>
              <span class="text-muted-soft">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted">Max:</span>
              <span class="font-bold text-ink">{{ durationMaxMs ?? "-" }}</span>
              <span class="text-muted-soft">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-lg bg-surface-soft p-4" style="order: 5">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-muted-soft"
                >TTFT</span
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.ttft')"
              />
            </div>
            <button
              v-if="!props.fullscreen"
        ...
        <div class="rounded-lg bg-surface-soft p-4" style="order: 3">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-muted-soft">{{
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.errors')"
              />
            </div>
            <button
              v-if="!props.fullscreen"
          ...
          <div class="mt-3 space-y-1 text-xs">
            ...
            <div class="flex justify-between">
              <span class="text-muted"
                >{{ t("admin.ops.businessLimited") }}:</span
              >
              <span class="font-bold text-ink">{{
        ...
        <div class="rounded-lg bg-surface-soft p-4" style="order: 6">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-muted-soft">{{
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.upstreamErrors')"
              />
            </div>
            <button
              v-if="!props.fullscreen"
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-muted"
                >{{ t("admin.ops.errorCountExcl429529") }}:</span
              >
              <span class="font-bold text-ink">{{
    ...
    <div v-if="overview" class="mt-2 border-t border-hairline-soft pt-4">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-surface-soft p-3">
          <div class="flex items-center gap-1">
            <div
              class="text-[10px] font-bold uppercase tracking-wider text-muted-soft"
            >
              CPU
            </div>
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="t('admin.ops.tooltips.cpu')"
            />
          </div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-muted">
        ...
        <div class="rounded-xl bg-surface-soft p-3">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="t('admin.ops.tooltips.memory')"
            />
          </div>
        ...
        <div class="rounded-xl bg-surface-soft p-3">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="t('admin.ops.tooltips.db')"
            />
          </div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-muted">
            {{ t("admin.ops.conns") }} {{ dbConnOpenValue ?? "-" }} /
            {{ dbMaxOpenConnsValue ?? "-" }} · {{ t("admin.ops.active") }}
            {{ dbConnActiveValue ?? "-" }} · {{ t("admin.ops.idle") }}
            {{ dbConnIdleValue ?? "-" }}
        ...
        <div class="rounded-xl bg-surface-soft p-3">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="t('admin.ops.tooltips.redis')"
            />
          </div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-muted">
            {{ t("admin.ops.conns") }} {{ redisConnTotalValue ?? "-" }} /
            {{ redisPoolSizeValue ?? "-" }}
        ...
        <div class="rounded-xl bg-surface-soft p-3">
          <div class="flex items-center gap-1">
            ...
            <HelpTooltip
              v-if="!props.fullscreen"
              :content="t('admin.ops.tooltips.goroutines')"
            />
          </div>
        ...
        <div class="rounded-xl bg-surface-soft p-3">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              ...
              <HelpTooltip
                v-if="!props.fullscreen"
                :content="t('admin.ops.tooltips.jobs')"
              />
            </div>
            <button
              v-if="!props.fullscreen"
    ...
    >
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-hairline-soft bg-canvas p-4"
        >
          ...
          <div
            class="mt-2 grid grid-cols-1 gap-2 text-xs text-body sm:grid-cols-2"
          >
            <div>
          ...
          <div
            v-if="hb.last_error"
            class="mt-3 rounded-lg bg-error/10 p-2 text-xs text-error"
          >
            {{ hb.last_error }}
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            type="datetime-local"
            class="w-full rounded-lg border border-hairline bg-canvas px-3 py-2 text-sm text-ink focus:border-accent-teal focus:outline-none focus:ring-1 focus:ring-primary-500"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-body mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-surface-card px-4 py-2 text-sm font-medium text-body hover:bg-hairline"
          ...
          <button
            type="button"
            class="rounded-lg bg-accent-teal px-4 py-2 text-sm font-medium text-on-primary hover:bg-primary-500"
