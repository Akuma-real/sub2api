...
const modalTitle = computed(() => {
  return props.errorType === "upstream"
...
const statusCodeSelectOptions = computed(() => {
  ...
  return [
    ...
    {
      value: "other",
      label: t("admin.ops.errorDetails.statusCodeOther") || "Other",
    },
  ];
});
...
const ownerSelectOptions = computed(() => {
  return [
    ...
    {
      value: "provider",
      label: t("admin.ops.errorDetails.owner.provider") || "provider",
    },
    {
      value: "client",
      label: t("admin.ops.errorDetails.owner.client") || "client",
    },
    {
      value: "platform",
      label: t("admin.ops.errorDetails.owner.platform") || "platform",
    },
  ];
});
...
const viewModeSelectOptions = computed(() => {
  return [
    {
      value: "errors",
      label: t("admin.ops.errorDetails.viewErrors") || "errors",
    },
    {
      value: "excluded",
      label: t("admin.ops.errorDetails.viewExcluded") || "excluded",
    },
...
const phaseSelectOptions = computed(() => {
  const options = [
    ...
    {
      value: "request",
      label: t("admin.ops.errorDetails.phase.request") || "request",
    },
    { value: "auth", label: t("admin.ops.errorDetails.phase.auth") || "auth" },
    {
      value: "routing",
      label: t("admin.ops.errorDetails.phase.routing") || "routing",
    },
    {
      value: "upstream",
      label: t("admin.ops.errorDetails.phase.upstream") || "upstream",
    },
    {
      value: "network",
      label: t("admin.ops.errorDetails.phase.network") || "network",
    },
    {
      value: "internal",
      label: t("admin.ops.errorDetails.phase.internal") || "internal",
    },
  ];
  return options;
});
        ...
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
          <div class="col-span-2 compact-select">
            <div class="relative group">
              ...
              >
                <svg
                  class="h-3.5 w-3.5 text-muted-soft transition-colors group-focus-within:text-accent-teal"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <input
                v-model="q"
                type="text"
                class="w-full rounded-lg border-hairline bg-surface-soft/50 py-1.5 pl-9 pr-3 text-xs font-medium text-body transition-all focus:border-accent-teal focus:bg-canvas focus:ring-2 focus:ring-primary-500/10"
