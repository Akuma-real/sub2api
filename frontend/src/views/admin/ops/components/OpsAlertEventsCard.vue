...
const statusOptions = computed(() => [
  ...
  { value: "resolved", label: t("admin.ops.alertEvents.status.resolved") },
  {
    ...
    label: t("admin.ops.alertEvents.status.manualResolved"),
  },
]);
...
async function loadFirstPage() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail || t("admin.ops.alertEvents.loadFailed"),
    );
    events.value = [];
    hasMore.value = false;
  } finally {
...
async function silenceAlert() {
  ...
  try {
    ...
    appStore.showError(
      err?.response?.data?.detail ||
        t("admin.ops.alertEvents.detail.silenceFailed"),
    );
  } finally {
...
async function manualResolve() {
  ...
  try {
    ...
    appStore.showSuccess(
      t("admin.ops.alertEvents.detail.manualResolvedSuccess"),
    );
    ...
    appStore.showError(
      err?.response?.data?.detail ||
        t("admin.ops.alertEvents.detail.manualResolvedFailed"),
    );
  } finally {
    detailActionLoading.value = false;
  }
...
function severityBadgeClass(severity: string | undefined): string {
  const s = String(severity || "")
    .trim()
    .toLowerCase();
  if (s === "p0" || s === "critical") return "bg-error/15 text-error ";
  if (s === "p1" || s === "warning") return "bg-accent-amber/15 text-warning ";
  if (s === "p2" || s === "info") return "bg-accent-teal/15 text-primary-700 ";
  if (s === "p3") return "bg-surface-card text-body ";
...
function statusBadgeClass(status: string | undefined): string {
  const s = String(status || "")
    .trim()
    .toLowerCase();
  if (s === "firing") return "bg-error/15 text-error ring-error/20 ";
  if (s === "resolved") return "bg-success/15 text-success ring-success/20 ";
  if (s === "manual_resolved") return "bg-canvas text-body ring-muted/20 ";
...
function formatStatusLabel(status: string | undefined): string {
  ...
  if (s === "manual_resolved")
    return t("admin.ops.alertEvents.status.manualResolved");
...
<template>
  <div class="rounded-lg bg-canvas p-6 shadow-sm ring-1 ring-ink/5">
    ...
    <div
      v-else-if="empty"
      class="rounded-xl border border-dashed border-hairline p-8 text-center text-sm text-muted"
    >
      {{ t("admin.ops.alertEvents.empty") }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-hairline">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <table class="min-w-full divide-y divide-hairline">
          <thead class="sticky top-0 z-10 bg-surface-soft">
            <tr>
              <th
                class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-muted"
              >
              ...
              <th
                class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-muted"
              >
          ...
          <tbody class="divide-y divide-hairline bg-canvas">
            <tr
              ...
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-body">
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-body">
                <div class="font-semibold truncate max-w-[360px]">
                  {{ row.title || "-" }}
                </div>
                <div
                  v-if="row.description"
                  class="mt-0.5 line-clamp-2 text-[11px] text-muted"
                >
                  {{ row.description }}
                </div>
              </td>
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-muted">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  <Icon
                    ...
                    class="text-success"
                  />
                  <Icon v-else name="ban" size="sm" class="text-muted-soft" />
                  <span class="text-[11px] font-bold text-body">
                    {{
                      row.email_sent
                        ? t("admin.ops.alertEvents.table.emailSent")
                        : t("admin.ops.alertEvents.table.emailIgnored")
        ...
        <div
          ...
          class="py-3 text-center text-xs text-muted-soft"
        >
          -
        </div>
      </div>
    ...
    >
      <div
        v-if="detailLoading"
        class="flex items-center justify-center py-10 text-sm text-muted"
      >
        {{ t("admin.ops.alertEvents.detail.loading") }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-muted">
      ...
      <div v-else class="space-y-5">
          ...
          >
            <div>
              ...
              <div
                ...
                class="mt-1 whitespace-pre-wrap text-xs text-body"
              >
            ...
            <div class="flex flex-wrap gap-2">
              <div
                class="flex items-center gap-2 rounded-lg bg-canvas px-2 py-1 ring-1 ring-hairline"
              >
                <span class="text-[11px] font-bold text-body">{{
                  t("admin.ops.alertEvents.detail.silence")
                }}</span>
                <Select
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div class="rounded-xl bg-surface-soft p-4">
            ...
            <div class="mt-1 flex flex-wrap items-center gap-2">
              ...
              <a
                class="inline-flex items-center gap-1 rounded-md bg-canvas px-2 py-1 text-[11px] font-bold text-body ring-1 ring-hairline hover:bg-surface-soft"
        ...
        <div class="rounded-xl border border-hairline bg-canvas p-4">
          ...
          <div
            v-else
            class="overflow-hidden rounded-lg border border-hairline-soft"
          >
            <table class="min-w-full divide-y divide-hairline-soft">
              <thead class="bg-surface-soft">
                <tr>
                  <th
                    class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-muted"
                  >
                    {{ t("admin.ops.alertEvents.table.time") }}
                  </th>
                  <th
              ...
              <tbody class="divide-y divide-hairline-soft">
                ...
                >
                  <td class="px-3 py-2 text-xs text-body">
