...
const viewModeOptions = computed(() => [
  ...
  {
    value: "pagination",
    label: t("admin.ops.openaiTokenStats.viewModePagination"),
  },
]);
...
async function loadData() {
  ...
  try {
    ...
    response.value = null;
    errorMessage.value =
      err?.message || t("admin.ops.openaiTokenStats.failedToLoad");
  } finally {
    loading.value = false;
  }
}
...
<template>
  <section class="card p-4 md:p-5">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <template v-else>
          ...
          <span class="text-xs text-muted">
            {{
              t("admin.ops.openaiTokenStats.pageInfo", {
                page,
                total: totalPages,
              })
            }}
    ...
    <div
      v-if="errorMessage"
      class="mb-4 rounded-lg bg-error/15 px-3 py-2 text-xs text-error"
    >
      {{ errorMessage }}
    </div>
    ...
    <div v-else class="space-y-3">
          ...
          <table class="min-w-full text-left text-xs md:text-sm">
            ...
            <tbody>
              <tr
                v-for="row in items"
                :key="row.model"
                class="border-b border-hairline-soft text-body last:border-b-0"
              >
      ...
      <div v-if="viewMode === 'topn'" class="mt-3 text-xs text-muted">
