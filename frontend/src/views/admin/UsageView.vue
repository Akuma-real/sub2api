<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="mb-4 flex gap-2 border-b border-hairline">
...
const breakdownFilters = computed(() => {
  ...
  if (filters.value.request_type != null)
    f.request_type = filters.value.request_type;
  if (filters.value.billing_type != null)
...
const exportToExcel = async () => {
  ...
  try {
    ...
    while (true) {
      const res = await adminUsageAPI.list(buildUsageListParams(p, 100, true), {
        signal: c.signal,
      });
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        (log.account_rate_multiplier ?? 1).toPrecision(4),
        log.total_cost?.toFixed(6) || "0.000000",
        log.actual_cost?.toFixed(6) || "0.000000",
        (
          (log.account_stats_cost ?? log.total_cost) *
          (log.account_rate_multiplier ?? 1)
        ).toFixed(6),
        log.first_token_ms ?? "",
        log.duration_ms,
        log.request_id || "",
...
const toRFC3339 = (
  ...
  d
    ? new Date(d + (endOfDay ? "T23:59:59.999" : "T00:00:00")).toISOString()
    : undefined;
...
const onErrPage = (p: number) => {
  errPage.value = p;
  loadAdminErrors();
};
const onErrPageSize = (s: number) => {
  errPageSize.value = s;
  errPage.value = 1;
  loadAdminErrors();
};
const openError = (id: number) => {
