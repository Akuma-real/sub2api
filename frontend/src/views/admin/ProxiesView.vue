<template>
  <AppLayout>
          ...
          >
            ...
            <template #cell-address="{ row }">
              <div class="flex items-center gap-1.5">
                ...
                <div class="relative">
                  <button
                    type="button"
                    class="rounded p-0.5 text-muted-soft hover:text-primary-600"
                  ...
                  <div
                    ...
                    class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-lg border border-hairline bg-canvas py-1 shadow-card"
                  >
                    <button
                      v-for="fmt in getCopyFormats(row)"
                      :key="fmt.label"
                      class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs hover:bg-surface-card"
                      @click.stop="copyFormat(fmt.value)"
                    >
                      <span class="truncate font-mono text-body">{{
                        fmt.label
                      }}</span>
                    </button>
                  </div>
            ...
            <template #cell-auth="{ row }">
              ...
              >
                <div class="flex flex-col text-xs">
                  <span v-if="row.username" class="text-body">{{
                    row.username
                  }}</span>
                  <span v-if="row.password" class="font-mono text-muted">
                    {{
                ...
                <button
                  v-if="row.password"
                  type="button"
                  class="ml-1 rounded p-0.5 text-muted-soft hover:text-body"
                  @click.stop="
                    visiblePasswordIds.has(row.id)
                      ? visiblePasswordIds.delete(row.id)
                      : visiblePasswordIds.add(row.id)
                  "
                >
                  <Icon
                    :name="visiblePasswordIds.has(row.id) ? 'eyeOff' : 'eye'"
                    size="sm"
                  />
                </button>
              </div>
            ...
            <template #cell-location="{ row }">
              <div class="flex items-center gap-2">
                ...
                />
                <span v-if="formatLocation(row)" class="text-sm text-body">
                  {{ formatLocation(row) }}
                </span>
                <span v-else class="text-sm text-muted-soft">-</span>
              </div>
            ...
            <template #cell-expiry="{ row }">
              <span v-if="!row.expires_at" class="text-sm text-muted-soft">{{ t("admin.proxies.neverExpires") }}</span>
              <div v-else class="flex flex-col gap-1 text-xs">
                <span class="text-body">{{ formatDateTime(row.expires_at) }}</span>
            ...
            <template #cell-account_count="{ row, value }">
              <button
                v-if="(value || 0) > 0"
                type="button"
                class="inline-flex items-center rounded bg-surface-card px-2 py-0.5 text-xs font-medium text-primary-700 hover:bg-hairline"
              ...
              <span
                v-else
                class="inline-flex items-center rounded bg-surface-card px-2 py-0.5 text-xs font-medium text-body-strong"
              >
            ...
            <template #cell-actions="{ row }">
              <div class="flex items-center gap-1">
                <button
                  ...
                  class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-success/15 hover:text-success disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <svg
                ...
                <button
                  ...
                  class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-accent-teal/10 hover:text-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <svg
    ...
    >
      <!-- Tab Switch -->
      <div class="mb-6 flex items-center justify-between gap-3 border-b border-hairline">
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-soft hover:text-body"
              @click="createPasswordVisible = !createPasswordVisible"
            >
              <Icon
                :name="createPasswordVisible ? 'eyeOff' : 'eye'"
                size="md"
              />
            </button>
          </div>
      ...
      <div v-else class="space-y-5">
        ...
        <div
          ...
          class="rounded-lg bg-surface-soft p-4"
        >
          <div class="flex items-center gap-4 text-sm">
            <div class="flex items-center gap-1.5">
              ...
              <span class="text-body">
                {{
                  t("admin.proxies.parsedCount", {
                    count: batchParseResult.valid,
                  })
                }}
              </span>
            </div>
            ...
            >
              ...
              <span class="text-warning">
                {{
                  t("admin.proxies.invalidCount", {
                    count: batchParseResult.invalid,
                  })
                }}
              </span>
            </div>
            ...
            >
              ...
              <span class="text-muted">
                {{
                  t("admin.proxies.duplicateCount", {
                    count: batchParseResult.duplicate,
                  })
                }}
              </span>
            </div>
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="rounded-lg border border-hairline bg-surface-soft p-4">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-body">
            <div>
              {{ t("admin.proxies.qualityExitIP") }}:
              {{ qualityReport.exit_ip || "-" }}
            </div>
            <div>
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-hairline">
          <table class="min-w-full divide-y divide-hairline text-sm">
            <thead class="bg-surface-soft text-xs uppercase text-muted">
              <tr>
                ...
                <th class="px-3 py-2 text-left">
                  {{ t("admin.proxies.qualityTableMessage") }}
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-hairline bg-canvas">
              <tr v-for="item in qualityReport.items" :key="item.target">
                ...
                <td class="px-3 py-2">
                  <span
                    class="badge"
                    :class="qualityStatusClass(item.status)"
                    >{{ qualityStatusLabel(item.status) }}</span
                  >
                </td>
                ...
                <td class="px-3 py-2 text-body">
                  ...
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-muted-soft"
                    >(cf-ray: {{ item.cf_ray }})</span
                  >
                </td>
              </tr>
            </tbody>
    ...
    <BaseDialog
      ...
      :title="
        t('admin.proxies.accountsTitle', { name: accountsProxy?.name || '' })
      "
      width="normal"
          ...
          <thead class="bg-surface-soft text-xs uppercase text-muted">
            <tr>
              <th class="px-4 py-2 text-left">
                {{ t("admin.proxies.accountName") }}
              </th>
              <th class="px-4 py-2 text-left">
                {{ t("admin.accounts.columns.platformType") }}
              </th>
              <th class="px-4 py-2 text-left">
                {{ t("admin.proxies.accountNotes") }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-hairline bg-canvas">
...
const columns = computed<Column[]>(() => [
  ...
  {
    key: "address",
    label: t("admin.proxies.columns.address"),
    sortable: false,
  },
  { key: "auth", label: t("admin.proxies.columns.auth"), sortable: false },
  {
    key: "location",
    label: t("admin.proxies.columns.location"),
    sortable: false,
  },
  {
    key: "account_count",
    label: t("admin.proxies.columns.accounts"),
    sortable: true,
  },
  {
    key: "latency",
    label: t("admin.proxies.columns.latency"),
    sortable: false,
  },
  {
    key: "expiry",
    label: t("admin.proxies.columns.expiry"),
    sortable: true,
  },
  {
    key: "created_at",
    label: t("admin.proxies.columns.createdAt"),
    sortable: true,
  },
  { key: "status", label: t("admin.proxies.columns.status"), sortable: true },
  {
    key: "actions",
    label: t("admin.proxies.columns.actions"),
    sortable: false,
  },
]);
  ...
  try {
    ...
    if (created > 0) {
      appStore.showSuccess(
        t("admin.proxies.batchImportSuccess", { created, skipped }),
      );
    } else {
...
const handleUpdateProxy = async () => {
  ...
  try {
    const updateData: any = {
      ...
      backup_proxy_id:
        editForm.fallback_mode === "proxy" ? editForm.backup_proxy_id : null,
...
const startQualityCheckingProxy = (proxyId: number) => {
  qualityCheckingProxyIds.value = new Set([
    ...qualityCheckingProxyIds.value,
    proxyId,
  ]);
};
...
const runProxyTest = async (proxyId: number, notify: boolean) => {
  ...
  try {
    ...
    if (notify) {
      if (result.success) {
        const message = result.latency_ms
          ? t("admin.proxies.proxyWorkingWithLatency", {
        ...
        appStore.showError(
          result.message || t("admin.proxies.proxyTestFailed"),
        );
      }
    }
    return result;
    ...
    const message =
      error.response?.data?.detail || t("admin.proxies.failedToTest");
...
const handleQualityCheck = async (proxy: Proxy) => {
  ...
  try {
    ...
    appStore.showSuccess(
      t("admin.proxies.qualityCheckDone", {
        score: result.score,
        grade: result.grade,
      }),
    );
    ...
    const message =
      error.response?.data?.detail || t("admin.proxies.qualityCheckFailed");
    appStore.showError(message);
...
const handleBatchTest = async () => {
  ...
  try {
    ...
    appStore.showSuccess(
      t("admin.proxies.batchTestDone", { count: ids.length }),
    );
    loadProxies();
  } catch (error: any) {
    appStore.showError(
      error.response?.data?.detail || t("admin.proxies.batchTestFailed"),
    );
...
const handleBatchQualityCheck = async () => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.proxies.batchQualityFailed"),
    );
...
const confirmBatchDelete = async () => {
  ...
  try {
    ...
    if (deleted > 0) {
      appStore.showSuccess(
        t("admin.proxies.batchDeleteDone", { deleted, skipped }),
      );
    } else if (skipped > 0) {
...
const openAccountsModal = async (proxy: Proxy) => {
  ...
  try {
    ...
    appStore.showError(
      error.response?.data?.detail || t("admin.proxies.accountsFailed"),
    );
