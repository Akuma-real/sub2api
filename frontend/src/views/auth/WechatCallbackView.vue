      ...
      <transition name="fade">
        ...
        >
          <div
            ...
            "
            class="rounded-xl border border-hairline bg-surface-soft p-4"
          >
            <div class="space-y-3">
              <div class="space-y-1">
              ...
              <label
                ...
                class="flex items-start gap-3 rounded-lg border border-hairline bg-canvas p-3 text-sm"
              >
                <input
              ...
              <div class="space-y-3">
                <div class="space-y-1">
                  ...
                  <p class="text-xs text-muted">
                    {{
                      hasCurrentAuthToken
                        ? t("auth.oauthFlow.bindCurrentAccountDescription", {
                            providerName,
                          })
                        : t("auth.oauthFlow.signInThenBindDescription", {
                            providerName,
                          })
                    }}
                  </p>
          ...
          <template v-else-if="needsBindLogin">
            <p class="text-sm text-body">
              {{
                t("auth.oauthFlow.bindSignInToExistingAccount", {
                  providerName,
                })
              }}
            </p>
...
async function handleBindCurrentAccount() {
  ...
  if (!startURL) {
    errorMessage.value =
      unavailableMessage || resolveWeChatOAuthUnavailableMessage();
    return;
  }
...
function applyAdoptionSuggestionState(
...
function extractPendingAccountEmail(
...
async function handleSubmitInvitation() {
  ...
  try {
    ...
    const completion: PendingWeChatCompletion = legacyPendingOAuthToken.value
      ? (
          await apiClient.post<PendingWeChatCompletion>(
...
async function handleCreateAccount(payload: PendingOAuthCreateAccountPayload) {
  ...
  isSubmitting.value = true;
  try {
    const { data } = await apiClient.post<PendingWeChatCompletion>(
...
onMounted(async () => {
  ...
  try {
    ...
    const completion =
      (await exchangePendingOAuthCompletion()) as PendingWeChatCompletion;
