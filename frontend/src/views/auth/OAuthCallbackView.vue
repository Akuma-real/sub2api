<template>
  <div class="min-h-screen bg-surface-soft px-4 py-10">
    <div class="mx-auto max-w-2xl">
      <div v-if="isProcessing" class="card p-6 text-center">
        ...
        ></div>
        <h1 class="mt-4 text-lg font-semibold text-ink">
      ...
      <div v-else-if="needsRegistrationCompletion" class="card p-6">
        ...
        <div class="mt-6 space-y-4">
          ...
          <p v-if="registrationError" class="text-sm text-error">
            {{ registrationError }}
          </p>
          <button
...
function redirectProviderCallbackToBackend(
...
function hasOAuthTokenResponse(
  value: Partial<OAuthTokenResponse>,
...
async function resumePendingEmailOAuth() {
  ...
  try {
    const completion =
      (await exchangePendingOAuthCompletion()) as EmailOAuthPendingCompletion;
    ...
    if (
      ...
      registrationEmail.value = String(
        completion.resolved_email || completion.email || "",
      ).trim();
