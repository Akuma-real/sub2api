<template>
  <AppLayout>
    ...
    >
      <p class="text-sm text-body">{{ t("payment.confirmCancel") }}</p>
      <template #footer>
        <div class="flex justify-end gap-3">
          <button class="btn btn-secondary" @click="cancelTargetId = null">
            {{ t("common.cancel") }}
          </button>
          <button
    ...
    >
      <div v-if="refundTarget" class="space-y-4">
        <div class="rounded-xl bg-surface-soft p-4">
          <div class="flex justify-between text-sm">
            ...
            <span class="font-mono text-ink">#{{ refundTarget.id }}</span>
          </div>
          <div class="mt-2 flex justify-between text-sm">
            <span class="text-muted">{{ t("payment.orders.amount") }}</span>
            <span class="text-ink">${{ refundTarget.amount.toFixed(2) }}</span>
          </div>
        </div>
        <div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <button class="btn btn-secondary" @click="refundTarget = null">
            {{ t("common.cancel") }}
          </button>
          <button
