<template>
  <div
    class="flex min-h-screen items-center justify-center bg-surface-soft px-4"
  >
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          ...
          <div
            v-else-if="isPending"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-accent-amber/15"
          >
            <div
              class="h-10 w-10 animate-spin rounded-full border-4 border-accent-amber border-t-transparent"
            ></div>
          </div>
          <div
            v-else
          ...
          <h2 class="mt-4 text-2xl font-bold text-ink">
            {{ statusTitle }}
          </h2>
        ...
        <!-- Order Info -->
        <div v-if="order" class="rounded-xl bg-canvas p-5 shadow-sm">
          <div class="space-y-3 text-sm">
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium text-ink">{{
                formatGatewayAmount(baseAmount)
              }}</span>
            </div>
            <div v-if="order.fee_rate > 0" class="flex justify-between">
              <span class="text-muted"
                >{{ t("payment.orders.fee") }} ({{ order.fee_rate }}%)</span
              >
              <span class="font-medium text-ink">{{
                formatGatewayAmount(feeAmount)
              }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{
            ...
            <div
              v-if="order.amount !== order.pay_amount"
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium text-ink">{{
                t(
                  paymentMethodI18nKey(order.payment_type),
                  normalizedOrderPaymentType(order.payment_type),
                )
              }}</span>
            </div>
            <div class="flex justify-between">
        ...
        <div v-else-if="returnInfo" class="rounded-xl bg-canvas p-5 shadow-sm">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="returnInfo.money" class="flex justify-between">
              ...
              <span class="font-medium text-ink">{{
                formatGatewayAmount(Number(returnInfo.money) || 0)
              }}</span>
            </div>
            <div v-if="returnInfo.type" class="flex justify-between">
              ...
              <span class="font-medium text-ink">{{
                t(
                  paymentMethodI18nKey(returnInfo.type),
                  normalizedOrderPaymentType(returnInfo.type),
                )
              }}</span>
            </div>
          </div>
...
function restoreRecoverySnapshot(context: {
  ...
  if (
    context.routeOutTradeNo &&
    restored.outTradeNo !== context.routeOutTradeNo
  ) {
    return null;
  }
...
async function resolveOrderFromResumeToken(
  resumeToken: string,
...
async function resolveOrderFromOutTradeNo(
  outTradeNo: string,
): Promise<PaymentOrder | null> {
  try {
...
onMounted(async () => {
  ...
  const hasLegacyFallbackContext =
    readRouteQueryString("trade_status").trim() !== "";
  const shouldUsePublicOutTradeNo =
    outTradeNo !== "" &&
    (hasLegacyFallbackContext || routeOrderId > 0 || orderId > 0);
  ...
  if (
    ...
    (!resumeToken || resumeTokenLookupFailed)
  ) {
