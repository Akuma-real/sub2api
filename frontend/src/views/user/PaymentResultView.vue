    ...
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          <div v-if="isSuccess"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-success/15">
          ...
          <div v-else-if="isPending"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-warning/15">
            <div class="h-10 w-10 animate-spin rounded-full border-4 border-warning border-t-transparent"></div>
          </div>
          <div v-else
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-error/15">
            <svg class="h-10 w-10 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <h2 class="mt-4 text-[22px] leading-tight text-ink">
            {{ statusTitle }}
          </h2>
        ...
        <div v-if="order" class="card p-6">
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-medium text-ink">#{{ order.id }}</span>
            </div>
            <div v-if="order.out_trade_no" class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.orderNo') }}</span>
              <span class="font-medium text-ink">{{ order.out_trade_no }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.baseAmount') }}</span>
              <span class="font-medium text-ink">{{ formatGatewayAmount(baseAmount) }}</span>
            </div>
            <div v-if="order.fee_rate > 0" class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</span>
              <span class="font-medium text-ink">{{ formatGatewayAmount(feeAmount) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-medium text-primary-700">{{ formatGatewayAmount(order.pay_amount) }}</span>
            </div>
            <div v-if="order.amount !== order.pay_amount" class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.creditedAmount') }}</span>
              <span class="font-medium text-ink">{{ order.order_type === 'balance' ? '$' + order.amount.toFixed(2) : formatGatewayAmount(order.amount) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-ink">{{ t(paymentMethodI18nKey(order.payment_type), normalizedOrderPaymentType(order.payment_type)) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.status') }}</span>
        ...
        <div v-else-if="returnInfo" class="card p-6">
          <div class="space-y-3 text-sm">
            <div v-if="returnInfo.outTradeNo" class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.orderId') }}</span>
              <span class="font-medium text-ink">{{ returnInfo.outTradeNo }}</span>
            </div>
            <div v-if="returnInfo.money" class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-medium text-ink">{{ formatGatewayAmount(Number(returnInfo.money) || 0) }}</span>
            </div>
            <div v-if="returnInfo.type" class="flex justify-between">
              <span class="text-muted">{{ t('payment.orders.paymentMethod') }}</span>
              <span class="font-medium text-ink">{{ t(paymentMethodI18nKey(returnInfo.type), normalizedOrderPaymentType(returnInfo.type)) }}</span>
            </div>
          </div>
        </div>
        <!-- Actions -->
