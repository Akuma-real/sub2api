<template>
  <div class="flex min-h-screen items-center justify-center bg-canvas p-4">
    <div
      class="w-full max-w-md space-y-4 rounded-lg border border-hairline bg-canvas p-6 shadow-card"
    >
      <!-- Amount + Order ID -->
      <div v-if="amount" class="text-center">
        <p class="text-3xl font-bold" :style="{ color: methodColor }">
          ¥{{ amount }}
        </p>
        <p v-if="orderId" class="mt-1 text-sm text-muted">
      ...
      <div v-if="error" class="space-y-3">
        <div
          class="rounded-lg border border-error/25 bg-error/15 p-3 text-sm text-error"
        >
          {{ error }}
        </div>
        <button
      ...
      <div v-else class="flex items-center justify-center py-8">
        ...
        />
        <span class="ml-3 text-sm text-muted">{{ hint }}</span>
      </div>
    </div>
  </div>
</template>
...
const DEFAULT_METHOD_COLOR = "#cc785c";
...
const methodColor = computed(
  () => METHOD_COLORS[method] || DEFAULT_METHOD_COLOR,
);
...
async function initStripe(clientSecret: string, publishableKey: string) {
  ...
  try {
    ...
    if (!stripe) {
      error.value = t("payment.stripeLoadFailed");
      return;
    }
    ...
    if (method === "alipay") {
      ...
      const { error: err } = await stripe.confirmAlipayPayment(clientSecret, {
      ...
      const result = await (
        stripe as unknown as StripeWithWechatPay
