<template>
  <component
    :is="isPopup ? 'div' : AppLayout"
    :class="isPopup ? 'min-h-screen bg-surface-soft ' : ''"
  >
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        <div
          class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-error/15"
        >
          <Icon name="exclamationCircle" size="xl" class="text-error" />
        </div>
        <h3 class="text-lg font-semibold text-ink">
          {{ t("payment.stripeLoadFailed") }}
        </h3>
        <p class="mt-2 text-sm text-muted">{{ initError }}</p>
        <button class="btn btn-primary mt-6" @click="router.push('/purchase')">
      ...
      <template v-else>
            ...
            <div class="flex flex-col items-center space-y-4">
              ...
              >
                <img
                  :src="wechatQrUrl"
                  alt="WeChat Pay QR"
                  class="h-56 w-56 rounded"
                />
                <div
                  class="pointer-events-none absolute inset-0 flex items-center justify-center"
                >
                  <span
                    class="rounded-full bg-primary-500 p-2 shadow-sm ring-2 ring-canvas"
                  >
                    <svg
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <p v-if="stripeError" class="mt-4 text-sm text-error">
              {{ stripeError }}
            </p>
            <button
              class="btn btn-stripe mt-6 w-full py-3 text-base"
              :disabled="stripeSubmitting || !stripeReady"
              @click="handleGenericPay"
            >
              <span
                v-if="stripeSubmitting"
...
onMounted(async () => {
  ...
  try {
    ...
    if (!stripe) {
      initError.value = t("payment.stripeLoadFailed");
      return;
    }
    ...
    initError.value = extractI18nErrorMessage(
      err,
      t,
      "payment.errors",
      t("payment.stripeLoadFailed"),
...
async function confirmWechatPay(stripe: Stripe, clientSecret: string) {
  const { paymentIntent, error } = await (
    stripe as Stripe & {
      confirmWechatPayPayment: (
        ...
        paymentIntent?: {
          status: string;
          next_action?: {
            wechat_pay_display_qr_code?: { image_data_url?: string };
          };
        };
    ...
    payment_method_options: {
      wechat_pay: { client: isMobileDevice() ? "mobile_web" : "web" },
    },
  });
