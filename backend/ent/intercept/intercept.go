...
// The OpenAIDualAttemptFunc type is an adapter to allow the use of ordinary function as a Querier.
type OpenAIDualAttemptFunc func(context.Context, *ent.OpenAIDualAttemptQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f OpenAIDualAttemptFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.OpenAIDualAttemptQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.OpenAIDualAttemptQuery", q)
}

// The TraverseOpenAIDualAttempt type is an adapter to allow the use of ordinary function as Traverser.
type TraverseOpenAIDualAttempt func(context.Context, *ent.OpenAIDualAttemptQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseOpenAIDualAttempt) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseOpenAIDualAttempt) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.OpenAIDualAttemptQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.OpenAIDualAttemptQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.OpenAIDualAttemptQuery:
		return &query[*ent.OpenAIDualAttemptQuery, predicate.OpenAIDualAttempt, openaidualattempt.OrderOption]{typ: ent.TypeOpenAIDualAttempt, tq: q}, nil
