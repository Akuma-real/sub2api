...
func (_q *RedeemCodeQuery) QueryVipLevel() *VIPLevelQuery {
...
func (_q *RedeemCodeQuery) WithVipLevel(opts ...func(*VIPLevelQuery)) *RedeemCodeQuery {
	query := (&VIPLevelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RedeemCodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RedeemCode, error) {
	...
	if query := _q.withVipLevel; query != nil {
		if err := _q.loadVipLevel(ctx, query, nodes, nil,
			func(n *RedeemCode, e *VIPLevel) { n.Edges.VipLevel = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RedeemCodeQuery) loadVipLevel(ctx context.Context, query *VIPLevelQuery, nodes []*RedeemCode, init func(*RedeemCode), assign func(*RedeemCode, *VIPLevel)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*RedeemCode)
	for i := range nodes {
		if nodes[i].VipLevelID == nil {
			continue
		}
...
func (_q *RedeemCodeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withVipLevel != nil {
			_spec.Node.AddColumnOnce(redeemcode.FieldVipLevelID)
		}
	}
