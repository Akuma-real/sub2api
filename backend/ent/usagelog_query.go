...
func (_q *UsageLogQuery) QueryVipLevel() *VIPLevelQuery {
...
func (_q *UsageLogQuery) WithVipLevel(opts ...func(*VIPLevelQuery)) *UsageLogQuery {
	query := (&VIPLevelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	...
	if query := _q.withVipLevel; query != nil {
		if err := _q.loadVipLevel(ctx, query, nodes, nil,
			func(n *UsageLog, e *VIPLevel) { n.Edges.VipLevel = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UsageLogQuery) loadVipLevel(ctx context.Context, query *VIPLevelQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *VIPLevel)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UsageLog)
	for i := range nodes {
...
func (_q *UsageLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withVipLevel != nil {
			_spec.Node.AddColumnOnce(usagelog.FieldVipLevelID)
		}
	}
