...
const apiKeyAuthSnapshotVersion = 13 // v13: include per-key OpenAI acceleration settings
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		AccelerationSettings: apiKey.AccelerationSettings.Normalize(),
		Quota:                apiKey.Quota,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		AccelerationSettings: snapshot.AccelerationSettings.Normalize(),
