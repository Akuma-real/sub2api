...
func TestBuildVisibleMethodSourceAvailability(t *testing.T) {
	...
	instances := []*dbent.PaymentProviderInstance{
		...
		{ProviderKey: payment.TypeMuyin, SupportedTypes: "alipay,wxpay"},
	...
	if !got[VisibleMethodSourceMuyinAlipay] {
		t.Fatalf("expected %q to be available", VisibleMethodSourceMuyinAlipay)
	}
	if !got[VisibleMethodSourceMuyinWechat] {
		t.Fatalf("expected %q to be available", VisibleMethodSourceMuyinWechat)
	}
}
